...
namespace MALL
{
    ...
    */
    ...
    public static class Defaults
    {
        ...
        public static string Def_Consolidated_VoucherLimit = "0";
        ...
        public static string G_MAXIMUMLPO_AMNT = "0";

        public static string G_DOCUMENT_URL = "";

        public static bool G_BLOCKPRR_FOC_ENTRY = false;

        public static int G_REORDERLPO_MAXCONFIRMDA = 7;

        public static bool G_IS_SALESTAX_APPLICABLE = true;

        public static bool G_SALES_AGAINST_QUOTATION = false;

        public static bool G_PRHCONF_WITH_DOC_UPLOAD = false;

        public static bool G_RETURN_AGAINST_SALES = false;

        public static bool G_COMMON_PRIVILEGE_FOR_CV_VOUCHERS = true;

        public static bool G_COSTCENTER_ENABLED_LOC = false;

        public static int G_NOTIFICATION_TIMER_TICK = 0;

        public static bool OVERSEASBUDGT_ENABLED_LOC = false;

        public static bool G_ITEMIMAGE_ENABLED_LOC = false;

        public static decimal G_PURCHASE_HIGH_MARGIN = 60;

        public static string G_ERP_MIN_SUPPORTED_VERSION = "";
                    ...
                    while (rs.Read())
                    {
                        ...
                        switch (rs["SETTING"].ToString())
                        {
                            ...
                            case "OVER_PURCHASE_LIMIT":
                                ...
                                break;
                            case "G_ACCT_CONSVCH_LIMIT":

                                if (rs["VALUE1"].ToString().Equals("Y"))
                                    Def_Consolidated_VoucherLimit = rs["VALUE2"].ToString();
                                break;
                            ...
                            case "EXPIRYDAYS_LPO":
                                ...
                                break;
                            case "G_REORDERLPO_MAXCONFIRMDA":
                                G_REORDERLPO_MAXCONFIRMDA = rs["VALUE1"].ToString() == "" ? 7 : int.Parse(rs["VALUE1"].ToString());
                                break;
                            case "G_MAXIMUMLPO_AMNT":
                                if (rs["VALUE1"].ToString().Equals("Y"))
                                {
                                    G_MAXIMUMLPO_AMNT = rs["VALUE2"].ToString() == "" ? "0" : rs["VALUE2"].ToString() == null ? "0" : rs["VALUE2"].ToString();
                                }
                                else
                                {
                                    G_MAXIMUMLPO_AMNT = "0";
                            ...
                            case "G_DOCUMENT_URL":
                                if (rs["VALUE1"].ToString().Equals("Y"))
                                {
                                    G_DOCUMENT_URL = rs["VALUE2"].ToString();
                                }
                                break;
                            case "SCALE_BARCODE_SERIES":
                                try
                                {
                                    Defaults.G_SCALE_BARCODE_FORMAT = null;
                                    string Format = rs["VALUE1"].ToString();
                                    Format = Format.Substring(1, Format.Length - 2);
                                    string[] F1 = Format.Split(new[] { ',' });
                                    ArrayList al = new ArrayList();
                                    foreach (string key in F1)
                                    {
                                        string key1 = key;
                                        key1 = key1.Replace(" ", "").Trim();
                                        if (key1.Contains("-"))
                                        {
                                            string[] F2 = key1.Split(new[] { '-' });
                                            int i = int.Parse(F2[0].Trim());
                                            int j = int.Parse(F2[1].Trim());
                                            for (int k = i; k <= j; k++)
                                            {
                                                al.Add(k);
                                            }
                                        ...
                                        {
                                            al.Add(int.Parse(key.Trim()));
                                        }
                                    }
                                    if (al.Count > 0)
                                    {
                                        Defaults.G_SCALE_BARCODE_FORMAT = new short[al.Count];
                                        int k = 0;
                                        foreach (object s in al.ToArray())
                                        {
                                            Defaults.G_SCALE_BARCODE_FORMAT[k++] = short.Parse(s + "");
                                        }
                                    }
                                }
                                catch (Exception ex)
                                {
                                    DataConnector.Message("Wrong Format For Scale Barcode\r\nExpected Format = {12 - 15,17 - 19,661}\r\nPN Part Number Start and Length,PRC Price start and length,BL Barcode length\r\n" + ex.Message);
                                }
                                break;
                            case "IS_SALESTAX_APPLICABLE":
                                if (rs["VALUE1"].ToString().Equals("Y"))
                                {
                                    G_IS_SALESTAX_APPLICABLE = true;
                                }
                                else
                                {
                                    G_IS_SALESTAX_APPLICABLE = false;
                                }
                                break;
                            case "SCALE_BARCODE_FORMAT":
                                try
                                {
                                    string Format = rs["VALUE1"].ToString();
                                    Format = Format.Substring(1, Format.Length - 2);
                                    ...
                                    foreach (string key in F1)
                                    {
                                        ...
                                        switch (Key1.Substring(0, 3))
                                        {
                                            case "PN:":
                                            case "PN=":
                                                t = Key1.Substring(3).Split(new[] { ',' });
                                                Defaults.G_SCALE_BARCODE_PN_X = int.Parse(t[0]);
                                                Defaults.G_SCALE_BARCODE_PN_Length = int.Parse(t[1]);
                                                break;
                                            case "PR:":
                                            case "PR=":
                                                t = Key1.Substring(3).Split(new[] { ',' });
                                                Defaults.G_SCALE_BARCODE_PRC_X = int.Parse(t[0]);
                                                Defaults.G_SCALE_BARCODE_PRC_Length = int.Parse(t[1]);
                                                break;
                                            case "BL:":
                                            case "BL=":
                                                t = Key1.Substring(3).Split(new[] { ',' });
                                                Defaults.G_SCALE_BARCODE_LENGTH = int.Parse(t[0]);
                                                break;
                                        }
                                    }
                                }
                                ...
                                break;
                            case "G_BLOCK_PRR_FOC_ENTRY":
                                if (rs["VALUE1"].ToString().Equals("Y"))
                                {
                                    G_BLOCKPRR_FOC_ENTRY = true;
                                }
                                break;
                            case "SALES_AGAINST_QUOTATION":
                                if (rs["VALUE1"].ToString().Equals("Y"))
                                {
                                    G_SALES_AGAINST_QUOTATION = true;
                                }
                                break;
                            case "PRHCONF_WITH_DOC_UPLOAD": // For Identifying grv confirmation with document uploading feature
                                G_PRHCONF_WITH_DOC_UPLOAD = rs["VALUE1"].ToString().Equals("Y") ? true : false;
                                break;
                            case "G_RETURN_AGAINST_SALES":
                                if (rs["VALUE1"].ToString().Equals("Y"))
                                {
                                    G_RETURN_AGAINST_SALES = true;
                                }
                                break;
                            case "COMMON_PRIVILEGE4VOUCHERS":
                                G_COMMON_PRIVILEGE_FOR_CV_VOUCHERS = rs["VALUE1"].ToString().Equals("N") ? false : true;
                                break;
                            case "COSTCENTER_ENABLED_LOC":
                                G_COSTCENTER_ENABLED_LOC = rs["VALUE1"].ToString().Equals("Y") ? true : false;
                                break;
                            case "NOTIFICATION_TIMER_TICK":
                                G_NOTIFICATION_TIMER_TICK = rs["VALUE1"].ToString().Equals("Y") ? int.Parse(rs["VALUE2"].ToString()) : 0;
                                break;
                            case "ITEMIMAGE_ENABLED_LOC":
                                if (rs["VALUE1"].ToString().Equals("Y"))
                                {
                                    G_ITEMIMAGE_ENABLED_LOC = true;
                                }
                                break;
                            case "OVERSEASBUDGT_ENABLED_LOC":
                                if (rs["VALUE1"].ToString().Equals("Y"))
                                {
                                    OVERSEASBUDGT_ENABLED_LOC = true;
                                }
                                break;
                            case "PURCHASE_HIGH_MARGIN":
                                if (rs["VALUE1"].ToString().Equals("Y"))
                                {
                                    G_PURCHASE_HIGH_MARGIN = int.Parse(rs["VALUE2"].ToString());
                                }
                                break;
                            case "ERP_MIN_SUPPORTED_VERSION":
                                if (rs["VALUE1"].ToString().Equals("Y"))
                                {
                                    G_ERP_MIN_SUPPORTED_VERSION = rs["VALUE2"].ToString();
                                }
                                break;
                        }
                    }
    ...
    public class DataConnector
    {
        ...
        public OracleConnection getPooledConnection(string MACHINE)
        {
            ...
            if (Defaults.Def_Conn == null || Defaults.Def_Conn.State == ConnectionState.Closed)
            {
                String strConnString = "User Id=" + strUser.Trim() + ";Password=" + strPass.Trim() + ";Data Source=" + ds.Trim() + ";Max Pool Size=500;";
                try
                {
                    Defaults.Def_Conn = new OracleConnection();
                    Defaults.Def_Conn.ConnectionString = strConnString.Trim();
        ...
        public OracleConnection getSGCPOSServerConnection(string MACHINE)
        {
            string ds = "";
            if (MACHINE == "SGCPOSSERVER")
            {
                ds = "SGCPOSSERVER";
                strUser = "mall";
                strPass = "mall";
        ...
        //NOTIFICATION : MUKESH 23/05/2023
        public bool AddToNotifications(string strNotificationCat, string strUserIdType, string strSubject, string strMessage, DateTime dtScheduleDate,
           string strRemarks, bool boolSms, bool boolEmail, bool boolApp, bool boolInternal, bool boolMySafari, OracleConnection oraConn, OracleTransaction oraTrns)
        {
            bool bReturn = false;
            string FD = "ID,NOTIFICATION_CAT,USER_ID_TYPE,SCHEDULED_DATE,SUBJECT,MESSAGE,REMARKS,SMS,EMAIL,APP,INTERNAL,MYSAFARI,STATUS";

            string clobMsg = StringToClob(strMessage);
            string scheduleDate = "to_date('" + dtScheduleDate.ToString() + "','DD/MON/YYYY HH:MI:SS AM')";

            string Sms = boolSms ? "Y" : "N";
            string Email = boolEmail ? "Y" : "N";
            string App = boolApp ? "Y" : "N";
            string Internal = boolInternal ? "Y" : "N";
            string MySafari = boolMySafari ? "Y" : "N";

            string VL = "SEQ_NOTIFICATION.nextval,'" + strNotificationCat + "','" + strUserIdType + "'," + scheduleDate + ",'" + strSubject + "'," + clobMsg + ",'" + strRemarks + "','" + Sms + "','" + Email + "','" + App + "','" + Internal + "','" + MySafari + "','NEW'";
            string strOracle = "INSERT INTO SYS_NOTIFICATIONS  " + "(" + FD.Trim() + ") VALUES (" + VL + ")";
            try
            {
                using (var oraCommand = oraConn.CreateCommand())
                {
                    oraCommand.CommandText = strOracle;
            ...
            catch (Exception exp)
            {
                ...
                {
                    MessageBox.Show("column not allowed here.....");
                }
                throw;
            }
            return (bReturn);
        ...
        public static string StringToClob(string message)
        {
            string ret = "";
            for (var i = 0; i < message.Length / 3500d; i++)
            {
                var len = message.Length - i * 3500 > 3500 ? 3500 : message.Length - i * 3500;
                ret += "to_clob('" + message.Substring(i * 3500, len) + "')||";
            }
        ...
        public static bool get_blockedStatus(string doctype, string loccode, string packing, string partnumber, OracleConnection oraConn)
        {
            ...
            try
            {
                string sql = " select * from INVT_PRODUCT_BLOCKING where loc_code='" + loccode + "' and (BLOCK_MODE='" + doctype + "' OR BLOCK_MODE='ALL') and PART_NUMBER_SUP='" + partnumber + "' and ( PACKING='" + packing + "' OR PACKING='ALL' ) order by BLOCK_MODE asc,ALL_PACKING_BLOCK asc,PACKING asc";
        ...
        public string GetDocSerial_AcctUpdated(string Loc_Code, string Doc_Type, string Doc_Year, OracleConnection con, OracleTransaction tr)
        {
            OracleCommand cmd = con.CreateCommand();
            OracleDataReader rs = null;
            cmd.CommandText = "SELECT TRIM(DOC_PREFIX)||(DOC_SERIAL-1) FROM COMN_DOCUMENTSERIALS WHERE  DOC_TYPE='" + Doc_Type + "' AND PROC_YEAR='" + Doc_Year + "'";
            try
            {
                if (!UpdateDocSerial_Acct(Doc_Type, Doc_Year, con, tr))
                {
            ...
            try
            {
                ...
                if (rs.HasRows)
                {
                    ...
                    if (rs.Read())
                    {
                        ...
                        if (ItemCode.Length <= Defaults.G_SCALE_BARCODE_PN_Length)
                        {
        ...
        public string GetReq_PartNumberSupOLD(string SupplierCode, OracleConnection Conn, OracleTransaction Tr)
        {
            string ItemCode;
            string temp;
            string SupplierPart;
            ...
            try
            {
                using (var cmd = Conn.CreateCommand())
                {
                   ...
                   + "Select max(to_number(SUBSTR(PART_NUMBER_SUP,5))+1) As PART_NUMBER_SUP From INVT_INVENTORYMASTER_REQ)";
                    ...
                    oSql = "Select SUPPLIERPART||(Case When LENGTH(PART_NUMBER_SERIAL)<2 Then LPAD(PART_NUMBER_SERIAL,2,'0') Else ''||PART_NUMBER_SERIAL End)  AS PART_NUMBER_SUP From" +
                         " (Select SUPPLIERPART,(Case When Max(PART_NUMBER_SUP)is null Then  1 Else Max(PART_NUMBER_SUP) End) As PART_NUMBER_SERIAL From " +
                         " (Select MAX(SUBSTR(SUPPLIER_CODE,-4,4)) AS SUPPLIERPART,MAX(SUBSTR(SUPPLIER_CODE,-4,4)), max(to_number(SUBSTR(PART_NUMBER_SUP,5))+1) As PART_NUMBER_SUP From INVT_INVENTORYMASTER WHERE SUPPLIER_CODE='" + SupplierCode + "' GROUP BY SUPPLIER_CODE" +
                         " UNION  Select MAX(SUBSTR(SUPPLIER_CODE,-4,4)) AS SUPPLIERPART,MAX(SUBSTR(SUPPLIER_CODE,-4,4)), max(to_number(SUBSTR(PART_NUMBER_SUP,5))+1) As PART_NUMBER_SUP From INVT_INVENTORYMASTER_REQ WHERE SUPPLIER_CODE='" + SupplierCode + "' GROUP BY SUPPLIER_CODE )GROUP BY SUPPLIERPART)";

                    //oSql = "Select SUPPLIERPART||(Case When LENGTH(PART_NUMBER_SERIAL)<2 Then LPAD(PART_NUMBER_SERIAL,2,'0') Else ''||PART_NUMBER_SERIAL End)  AS PART_NUMBER_SUP From" +
                    oSql = " (Select SUPPLIERPART,(Case When Max(PART_NUMBER_SUP)is null Then  1 Else Max(PART_NUMBER_SUP) End) As PART_NUMBER_SERIAL From " +
                         " (Select MAX(SUBSTR(SUPPLIER_CODE,-4,4)) AS SUPPLIERPART,MAX(SUBSTR(SUPPLIER_CODE,-4,4)), max(to_number(SUBSTR(PART_NUMBER_SUP,5))+1) As PART_NUMBER_SUP From INVT_INVENTORYMASTER WHERE SUPPLIER_CODE='" + SupplierCode + "' GROUP BY SUPPLIER_CODE" +
                         " UNION  Select MAX(SUBSTR(SUPPLIER_CODE,-4,4)) AS SUPPLIERPART,MAX(SUBSTR(SUPPLIER_CODE,-4,4)), max(to_number(SUBSTR(PART_NUMBER_SUP,5))+1) As PART_NUMBER_SUP From INVT_INVENTORYMASTER_REQ WHERE SUPPLIER_CODE='" + SupplierCode + "' GROUP BY SUPPLIER_CODE )GROUP BY SUPPLIERPART)";
                    ...
                    using (var rs = cmd.ExecuteReader())
                    {
                        ...
                        if (rs.HasRows)
                        {
                            if (rs.Read())
                            {
                                ItemCode = rs["SUPPLIERPART"].ToString() + (int.Parse(rs["PART_NUMBER_SERIAL"].ToString()) < 10 ? rs["PART_NUMBER_SERIAL"].ToString().PadLeft(2, '0') : rs["PART_NUMBER_SERIAL"].ToString());
                                if (ItemCode.Length <= Defaults.G_SCALE_BARCODE_PN_Length)
                                {
                                    ItemNumber = Convert.ToInt64(ItemCode);
                                    ItemNumber++;
        ...
        */
        ...
        public enum TransferTypes
        {
            ...
            PRICE_BATCH, TRANREQ, TENPRC, GPO, REM, CSUBREQ, CITEMSREQ, CPACKINGREQ, CBARCODEREQ, CBUNDLEREQ, CLUGGREQ, CSUPADDL, CNEWDIV, IADREQ, PRD
        }
            ...
            try
            {
                ...
                switch (Typ)
                {
                    ...
                    case TransferTypes.IADREQ:
                        Priority = 4;
                        break;
                    case TransferTypes.PRD://17112022
                        Priority = 5;
                        break;
                }
        ...
        public bool SendEmail(string Address, string FromAddr, string Subject, string BodyText, string Attachments, string pwd, bool viaGoogleApi, string Def_MailID_From)
        {
            ...
            else
            {
                try
                {
                    ...
                    int port = 587; bool SSL = true, Outlook1st = true;
                    string sfrom = "";
                    ...
                    if (Outlook1st && SendMailOutLook(Address, FromAddr, Subject, BodyText, Attachments))
                    {
                        return true;
                    }
                    else
                    {
                        Message("Sending from outlook failed!!! " + Outlook1st + ":" + Address + ":" + FromAddr + ":" + Subject);
                        throw new Exception("Sending from outlook failed!!!");
                    }
                    //}
                    #region OLD CODE 
                    //bool ErrSend = false;
                ...
                {
                    DataConnector.Message(ex.ToString());
                    return false;
                }
            }
            return false;
        ...
        public bool AddPricechangeHistory(string PART_NUMBER_SUP, string Doc_Date, string PACKING, string PRICE_CODE, string OLD_PRICE_CODE, string EFF_DATE, string VALID_UPTO, string CHANGE_PERCENT, string BAR_CODE, decimal RETAIL_PRICE, decimal WHOLESALE_RATE, decimal WHOLESALE_RATE1, decimal WHOLESALE_RATE2, decimal BUYING_RATE, decimal OLD_BUYING_RATE, decimal OLD_RETAIL_PRICE, decimal OLD_WHOLESALE_RATE, decimal OLD_WHOLESALE_RATE1, decimal OLD_WHOLESALE_RATE2, string OLD_BAR_CODE, string DOC_YEAR, string LOC_CODE, string CONFIRMED_ID, string EMP_ID, string Entry_Mode_PRC_BAR_NEWi_NEWp_DBN_BARd, string Remarks, string LPO_PRICE, string OLD_LPO_PRICE, OracleConnection con, OracleTransaction Tr)
        {
            if (Entry_Mode_PRC_BAR_NEWi_NEWp_DBN_BARd == "")
            {
                throw (new Exception("Mention Entry Mode"));
            ...
            string sql = "insert into invt_pricechange_history(PART_NUMBER_SUP,PACKING,PRICE_CODE,OLD_PRICE_CODE,EFF_DATE,VALID_UPTO,CHANGE_PERCENT,BAR_CODE,RETAIL_PRICE,WHOLESALE_RATE,WHOLESALE_RATE1,WHOLESALE_RATE2,BUYING_RATE,OLD_BUYING_RATE,OLD_RETAIL_PRICE,OLD_WHOLESALE_RATE,OLD_WHOLESALE_RATE1,OLD_WHOLESALE_RATE2,OLD_BAR_CODE,DOC_YEAR,LOC_CODE,CONFIRMED_ID, EMP_ID,doc_date,ENTRY_MODE,REMARKS,LPO_PRICE,OLD_LPO_PRICE)" +
                " values('" + PART_NUMBER_SUP + "','" + PACKING + "','" + PRICE_CODE + "','" + (OLD_PRICE_CODE == "" ? null : OLD_PRICE_CODE) + "',to_date('" + (EFF_DATE == "" ? null : EFF_DATE) + "','DD/MON/YYYY HH:MI:SS AM'),to_date('" + (VALID_UPTO == "" ? null : VALID_UPTO) + "','DD/MON/YYYY HH:MI:SS AM')," + (CHANGE_PERCENT == "" ? "0" : CHANGE_PERCENT) + ",'" + BAR_CODE + "'," + RETAIL_PRICE + "," + WHOLESALE_RATE + "," + WHOLESALE_RATE1 + "," + WHOLESALE_RATE2 + "," + BUYING_RATE + "," + OLD_BUYING_RATE + "," + OLD_RETAIL_PRICE + "," + OLD_WHOLESALE_RATE + "," + OLD_WHOLESALE_RATE1 + "," + OLD_WHOLESALE_RATE2 + ",'" + OLD_BAR_CODE + "','" + DOC_YEAR + "','" + LOC_CODE + "','" + CONFIRMED_ID + "','" + EMP_ID + "',to_date('" + Doc_Date + "','DD/MON/YYYY HH:MI:SS AM'),'" + Entry_Mode_PRC_BAR_NEWi_NEWp_DBN_BARd + "','" + Remarks + "'," + LPO_PRICE + "," + OLD_LPO_PRICE + ")";
            if (ExecuteCmd(sql, con, Tr))
            {
                AddDataTransfer(PRICE_CODE, "A", LOC_CODE, Defaults.Def_Base_LOC, TransferTypes.PRICE_HISTORY, con, Tr);
                return true;
        ...
        public bool GetStock(string LocCode, string PartNumber, int Zero, bool IncludeHomeDelivery, out string stock, OracleConnection oraConn)
        {
            OracleCommand cmd3 = oraConn.CreateCommand();
            OracleCommand cmd4 = oraConn.CreateCommand();
            OracleDataReader rsS = null;
            OracleDataReader rsH = null;
            cmd3.CommandText = "SELECT Round((CB_QTY/PACK_MULTIPLY_QTY),2)as Stock FROM  INVT_INVENTORYBALANCE WHERE LOC_CODE='" + LocCode + "' AND PART_NUMBER_SUP='" + PartNumber + "' and ZERO_SERIAL=" + Zero + "";
            cmd4.CommandText = "Select Nvl(Round(Sum((D.TOTAL_QTY*D.PACK_MULTIPLY_QTY)/(Case When D.ZERO_SERIAL=0 Then 1 Else to_number(D.PACK_MULTIPLY_QTY) End)),2),0) As PENDING_STOCK From INVT_PURCHASEORDERMAIN M Inner Join INVT_PURCHASEORDERSUB D On M.DOC_NO=D.DOC_NO Where M.LOC_CODE='" + LocCode + "' And HOME_DELIVERY='Y' And CONFIRMATION_STATUS='Y' And RECEIVED_ID Is Null And DELIVERY_LOCATION='" + LocCode + "' And D.PART_NUMBER_SUP='" + PartNumber + "' And D.ZERO_SERIAL=" + Zero + ""; //DELIVERY_LOCATION
            try
            {
                rsS = cmd3.ExecuteReader();
                string SD = "0";
                ...
                rsH = cmd4.ExecuteReader();
                string HD = "0";
                if (rsH.Read())
                {
                    HD = rsH.GetValue(0).ToString();
                }

                stock = SD;
                if (IncludeHomeDelivery)
                {
                    stock = "" + (decimal.Parse(SD) + decimal.Parse(HD));
                }
            }
            catch { throw; }
            finally
        ...
        public void Log_Audit_POS_Server(string Loc_Code, string Form_name, string Action_type, string Action, string Description, string Doc_No, string Doc_Date, string Doc_Type, OracleConnection oraConn, OracleTransaction Tr)
        {
            if (Form_name.Length > 30) Form_name = Form_name.Substring(0, 29);
            if (Action.Length > 50) Action = Action.Substring(0, 49);
            if (Action_type.Length > 1) Action_type = Action_type.Substring(0, 1);
            if (Description.Length > 255) Description = Description.Substring(0, 254);
            if (Doc_No != null && Doc_No.Length > 200) Doc_No = Doc_No.Substring(0, 199);
            if (Doc_Type.Length > 6) Doc_Type = Doc_Type.Substring(0, 5);
            ...
            if (oraConn == null)
            {
                try
                {
                    oraConn = getPooledConnection(Defaults.Def_POS_DB);
                    oraConn.Open();
                    LocalCon = true;
                }
                catch { return; }
            ...
            try
            {
                ...
                fields = "LOC_CODE_ORIGINAL,LOC_CODE,SYSTEM_NAME,IPADDRESS,OS,SYSTEM_USER,SERVER_TIME,LOCAL_TIME,APPLICATION,APPLICATION_LOGIN,FORM_NAME,ACTION_TYPE,ACTION,DESCRIPTION,DOC_NO,DOC_DATE,DOC_YEAR,DOC_TYPE";
                Values = "'" + Defaults.Def_LOC + "','" + Loc_Code + "','" + Environment.MachineName + "','" + myIP + "','" + Environment.OSVersion + "','" + Environment.UserName + "', SYSDATE,to_date('" + localDate + "','DD/MON/YYYY HH:MI:SS AM'),'" + Application.ProductName + "','" + Defaults.Def_User + "','" + Form_name + "','" + Action_type + "','" + Action + "','" + Description + "','" + Doc_No + "','" + DateTime.Parse(Doc_Date).ToString("dd/MMM/yyyy") + "','" + Defaults.Def_YEAR + "','" + Doc_Type + "'";
                strOracle = "INSERT INTO  SYS_AUDIT  (" + fields.Trim() + ") VALUES (" + Values + ")";

                oraCommand = oraConn.CreateCommand();
                oraCommand.CommandText = strOracle;
                if (Tr != null) { oraCommand.Transaction = Tr; }
                int r = oraCommand.ExecuteNonQuery();
            }
            catch (Exception ex)
            {
