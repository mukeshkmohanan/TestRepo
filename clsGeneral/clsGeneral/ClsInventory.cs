...
namespace MALL
{
    public class ClsInventory
    {
        ...
        public int GetZeroSerial(string Part_Number_Sup, string Packing)
        {
            ...
            decimal bu = 0;
            string a, b, c, d, e;
            GetPackingOrder(Part_Number_Sup, Packing, out ZeroSerial, out a, out b, out c, out d, out e, out bu);
            return ZeroSerial;
        ...
        public int GetPackingOrder(string Part_Number_Sup, string Packing)
        {
            ...
            decimal bu = 0;
            string a, b, c, d, e;
            return GetPackingOrder(Part_Number_Sup, Packing, out ZeroSerial, out a, out b, out c, out d, out e, out bu);
        }
            ...
            try
            {
                ...
                while (rs.Read())
                {
                    if (rs[2].Equals(Packing))
                    {
                        ...
                        buying_rate = decimal.Parse(rs["buying_rate"].ToString());
        ...
        */
        ...
        public bool UpdateStock(string Loc_Code, string Part_Number_Sup, string Packing, string Item_Type, string Doc_No, string Doc_Type, string GRV_TYPE, decimal QTY_LUQ, decimal Val, decimal FOC_LUQ, decimal PackQty, decimal Pack_Multiply_Qty, string Doc_Year, string Doc_Date, string To_Loc_Code, string Price_Code, decimal BUYING_RATE)
        {
            return UpdateStock(Loc_Code, Part_Number_Sup, Packing, Item_Type, Doc_No, Doc_Type, GRV_TYPE, QTY_LUQ, Val, FOC_LUQ, PackQty, Pack_Multiply_Qty, Doc_Year, Doc_Date, To_Loc_Code, 0, true, Price_Code, BUYING_RATE);
        }
        ...
        {
            return UpdateStock(Loc_Code, Part_Number_Sup, Packing, Item_Type, Doc_No, Doc_Type, GRV_TYPE, QTY_LUQ, Val, FOC_LUQ, PackQty, Pack_Multiply_Qty, Doc_Year, Doc_Date, To_Loc_Code, 0, true, "", 0);
        }
        public bool UpdateStock(string Loc_Code, string Part_Number_Sup, string Packing, string Item_Type, string Doc_No, string Doc_Type, string GRV_TYPE, decimal QTY_LUQ, decimal Val, decimal FOC_LUQ, decimal PackQty, decimal Pack_Multiply_Qty, string Doc_Year, string Doc_Date, string To_Loc_Code, decimal Unit_Rate, string Price_Code, decimal BUYING_RATE)
        {
            return UpdateStock(Loc_Code, Part_Number_Sup, Packing, Item_Type, Doc_No, Doc_Type, GRV_TYPE, QTY_LUQ, Val, FOC_LUQ, PackQty, Pack_Multiply_Qty, Doc_Year, Doc_Date, To_Loc_Code, Unit_Rate, true, Price_Code, BUYING_RATE);
        }
        ...
        {
            return UpdateStock(Loc_Code, Part_Number_Sup, Packing, Item_Type, Doc_No, Doc_Type, GRV_TYPE, QTY_LUQ, Val, FOC_LUQ, PackQty, Pack_Multiply_Qty, Doc_Year, Doc_Date, To_Loc_Code, Unit_Rate, true, "", 0);
        }

        public bool UpdateStock(string Loc_Code, string Part_Number_Sup, string Packing, string Item_Type, string Doc_No, string Doc_Type, string GRV_TYPE, decimal QTY_LUQ, decimal Val, decimal FOC_LUQ, decimal PackQty, decimal Pack_Multiply_Qty, string Doc_Year, string Doc_Date, string To_Loc_Code, decimal Unit_Rate, bool boolAddToTranStock, string Price_Code, decimal BUYING_RATE, bool ForgetBuyingRate = false, decimal SALE_VAL_WO_TAX = 0, decimal avg_Lc = 0, bool Do_not_ConsiderLInkedItem = false, bool SKIP_LC_UPD_OF_ITEM = false)
        {
            ...
            try
            {
                ...
                if (BUYING_RATE == 0 || ForgetBuyingRate) BUYING_RATE = Buying_rate;
                if (this.Price_Code == "") this.Price_Code = Price_Code;
                else Price_Code = this.Price_Code;
                if (Linked_Item != "") Item_Type = "L";
                OracleCommand cmd = null;
                OracleDataReader rs = null;
                string SysDate = Doc_Date;
                SysDate = clsGen.SysDate(con, tr, false);
                ...
                switch (Item_Type.ToUpper())
                {
                    case "G":
                        ...
                        UpdateItemTran(Loc_Code, TranDate, Doc_Type, Part_Number_Sup, QTY_LUQ, Val, FOC_LUQ, Pack_Multiply_Qty, Packing, To_Loc_Code, SysDate, ZeroSerial, PackingOrder, Unit_Rate, BUYING_RATE, true, SALE_VAL_WO_TAX);
                        UpdateStockBalance(Loc_Code, Doc_Type, Part_Number_Sup, QTY_LUQ, Val, To_Loc_Code, Doc_Date, SysDate, PackingOrder, ZeroSerial, Packing, BUYING_RATE, (int)Pack_Multiply_Qty, true, SALE_VAL_WO_TAX);
                        break;
                    // return true;
                    case "L":
                        if (Doc_Type.Equals("SAL") || Doc_Type.Equals("SRT"))
                        {
                            ...
                            string tmpPn = Part_Number_Sup;
                            decimal TLuq = QTY_LUQ;
                            ...
                            //}
                            sql = "select distinct B.part_number_sup,cb_qty,c.pack_multiply_qty,a.packing,last_grv_date, case when b.part_number_sup='" + tmpPn + "' then '1' else null end as SRT  from (invt_itemtran_dtls a INNER JOIN invt_inventorybalance b on a.loc_code=b.loc_code and a.part_number_sup=b.part_number_sup and a.zero_serial=b.zero_serial) inner join invt_itempacking c on a.part_number_sup=c.part_number_sup  and a.zero_serial=c.zero_serial WHERE a.loc_code='" + Loc_Code + "' and c.linked_ITEM='" + Linked_Item + "' " + zz + " order by SRT asc nulls last, last_grv_date " + Asc;
                            ...
                            try
                            {
                                ...
                                while (rs1.Read())
                                {
                                    ...
                                    if (Balance < 0)
                                    {
                                        UpdateTranDtls(rs1["part_number_sup"].ToString(), Doc_Type, Loc_Code, Doc_Date, RP * Stock, Stock, Packing, Doc_No, "", FOC_LUQ, PackingOrder, ZeroSerial, Pack_Multiply_Qty);
                                        UpdateItemTran(Loc_Code, TranDate, Doc_Type, rs1["part_number_sup"].ToString(), Stock, RP * Stock, FOC_LUQ, Pack_Multiply_Qty, Packing, "", SysDate, ZeroSerial, PackingOrder, Unit_Rate, BUYING_RATE);
                                        UpdateStockBalance(Loc_Code, Doc_Type, rs1["part_number_sup"].ToString(), Stock, RP * Stock, "", ZeroSerial, PackingOrder, Packing, BUYING_RATE, (int)Pack_Multiply_Qty);
                                        //Update QTY_LUQ = "stock" 
                                        QTY_LUQ = QTY_LUQ - Stock;
                                    }
                                    else
                                    {
                                        UpdateTranDtls(rs1["part_number_sup"].ToString(), Doc_Type, Loc_Code, Doc_Date, RP * QTY_LUQ, QTY_LUQ, Packing, Doc_No, "", FOC_LUQ, PackingOrder, ZeroSerial, Pack_Multiply_Qty);
                                        UpdateItemTran(Loc_Code, TranDate, Doc_Type, rs1["part_number_sup"].ToString(), QTY_LUQ, RP * QTY_LUQ, FOC_LUQ, Pack_Multiply_Qty, Packing, "", SysDate, ZeroSerial, PackingOrder, Unit_Rate, BUYING_RATE);
                                        UpdateStockBalance(Loc_Code, Doc_Type, rs1["part_number_sup"].ToString(), QTY_LUQ, RP * QTY_LUQ, "", ZeroSerial, PackingOrder, Packing, BUYING_RATE, (int)Pack_Multiply_Qty);
                                        QTY_LUQ = 0;
                                        break;
                                    }
                                }
                                ...
                                {
                                    UpdateTranDtls(Part_Number_Sup, Doc_Type, Loc_Code, Doc_Date, RP * QTY_LUQ, QTY_LUQ, Packing, Doc_No, "", FOC_LUQ, PackingOrder, ZeroSerial, Pack_Multiply_Qty);
                                    UpdateItemTran(Loc_Code, TranDate, Doc_Type, Part_Number_Sup, QTY_LUQ, RP * QTY_LUQ, FOC_LUQ, Pack_Multiply_Qty, Packing, "", SysDate, ZeroSerial, PackingOrder, Unit_Rate, Buying_rate);
                                    UpdateStockBalance(Loc_Code, Doc_Type, Part_Number_Sup, QTY_LUQ, RP * QTY_LUQ, "", ZeroSerial, PackingOrder, Packing, BUYING_RATE, (int)Pack_Multiply_Qty);
                                }
                                rs1.Close();
                                cmd1.Dispose();
                            }
                            catch (Exception eq) { throw (eq); }
                            finally { if (rs1 != null) rs1.Dispose(); cmd1.Dispose(); QTY_LUQ = TLuq; }
                        }
                        else
                        {
                            UpdateTranDtls(Part_Number_Sup, Doc_Type, Loc_Code, Doc_Date, Val, QTY_LUQ, Packing, Doc_No, To_Loc_Code, FOC_LUQ, PackingOrder, ZeroSerial, Pack_Multiply_Qty);
                            UpdateItemTran(Loc_Code, TranDate, Doc_Type, Part_Number_Sup, QTY_LUQ, Val, FOC_LUQ, Pack_Multiply_Qty, Packing, To_Loc_Code, SysDate, ZeroSerial, PackingOrder, Unit_Rate, BUYING_RATE);
                            UpdateStockBalance(Loc_Code, Doc_Type, Part_Number_Sup, QTY_LUQ, Val, To_Loc_Code, Doc_Date, SysDate, PackingOrder, ZeroSerial, Packing, BUYING_RATE, (int)Pack_Multiply_Qty);
                    ...
                    case "B":
                        ...
                        try
                        {
                            ...
                            {
                                if (Doc_Type == "PRH" || Doc_Type == "PRR") throw (new Exception("Invalid Doc Type For Bundle Item " + Part_Number_Sup));

                                try
                                {
                                    ApplayBundleValue(Part_Number_Sup, con, tr, (Val / QTY_LUQ));
                                }
                            ...
                            if (Doc_Type == "PRH" || Doc_Type == "PRR" || Doc_Type == "OB" || Doc_Type == "GTV" || Doc_Type == "ADJ")
                            {
                                cmd.CommandText = "select Pack_Multiply_Qty,asy_qty,item_bundle_Cost,packing,part_number_sup,count(*) over() as cnt from invt_assemblysub where ASY_part_number_sup='" + Part_Number_Sup + "'";
                            }
                            else
                            {
                                cmd.CommandText = "select Pack_Multiply_Qty,asy_qty,item_bundle_value,packing,part_number_sup,count(*) over() as cnt from invt_assemblysub where ASY_part_number_sup='" + Part_Number_Sup + "'";
                            }
                            ...
                            {
                                it++;
                                testVal += (QTY_LUQ * rs.GetDecimal(2));
                                Corr = 0;

                                if (rs.GetDecimal(5) == it)
                                {
                                    if (testVal != Val)
                                    {
                                        Corr = testVal - Val;
                                ...
                                if (Val != 0)
                                {
                                    decimal P = (((QTY_LUQ * rs.GetDecimal(2)) - Corr) / Val) * 100;
                                    pt += P;
                                    VNT = Math.Round(SALE_VAL_WO_TAX * P / 100, 3);
                                }

                                UpdateStock(Loc_Code, rs["part_number_sup"].ToString(), rs["packing"].ToString(), "", Doc_No, Doc_Type, "", (QTY_LUQ * rs.GetDecimal(0) * rs.GetDecimal(1)), (QTY_LUQ * rs.GetDecimal(2)) - Corr, (FOC_LUQ * rs.GetDecimal(0) * rs.GetDecimal(1)), rs.GetDecimal(1), rs.GetDecimal(0), Doc_Year, Doc_Date, To_Loc_Code, Unit_Rate, false, Price_Code, BUYING_RATE, true, VNT);
                            }

                            if (Val != 0 && (testVal - Corr != Val || Math.Round(pt, 3) != 100))
                            {
                                throw (new Exception(" Bundle Sub Items SALE VALUE MISMATCH " + Loc_Code + " - " + Part_Number_Sup + " " + Packing));
                            }

                            if (SubItemCount <= 0)
                            {
                                throw (new Exception(" Bundle Sub Items Not Found " + Loc_Code + " - " + rs["part_number_sup"].ToString() + " " + rs["packing"].ToString()));
                            }
                    ...
                    //    try
                    //    {
                    //        ApplayProductionCost(Part_Number_Sup, con, tr, 0, Loc_Code);

                    //        if (Doc_Type == "SAL" || Doc_Type == "SRT")
                    //        {
                    //            decimal itStock = getStock(Part_Number_Sup, Loc_Code, ZeroSerial);
                    //            UpdateItemTran(Loc_Code, TranDate, Doc_Type, Part_Number_Sup, QTY_LUQ, Val, FOC_LUQ, Pack_Multiply_Qty, Packing, To_Loc_Code, SysDate, ZeroSerial, PackingOrder, Unit_Rate, BUYING_RATE, true, SALE_VAL_WO_TAX, false);
                    //            UpdateStockBalance(Loc_Code, Doc_Type, Part_Number_Sup, QTY_LUQ, Val, To_Loc_Code, Doc_Date, SysDate, PackingOrder, ZeroSerial, Packing, BUYING_RATE, (int)Pack_Multiply_Qty, true, SALE_VAL_WO_TAX, false);

                    //            if (!clsGen.Exists("select distinct part_number_sup from INVT_PRODUCTION_TEMP_MASTER where part_number_sup='" + Part_Number_Sup + "' and zero_serial=" + ZeroSerial + " and active='Y' and POST_PRODUCTION='Y' And LOC_CODE='" + Loc_Code + "'", con, tr))
                    //            {
                    //                if (itStock > 0 && Doc_Type == "SAL" && itStock >= QTY_LUQ + FOC_LUQ)
                    //                {
                    //                    Item_Type = "G"; // SKIP END STOCK IN MAIN
                    //                }
                    //                else if (Doc_Type == "SAL" && itStock < QTY_LUQ + FOC_LUQ)
                    //                {

                    //                    OracleDataReader rsb = clsGen.GetValue(con, tr, "SELECT A.TOTAL_UNIT_COST AS LUP,A.LUQ AS MASTERLUQ,A.NET_PRODUCT_COST AS NET_PRODUCT_COST,B.PART_NUMBER_SUP,B.ZERO_SERIAL,B.PACKING,B.PACK_MULTIPLY_QTY,B.LUQ AS SUBLUQ,(B.LUQ/A.QTY) AS PROP_LUQ,B.UNIT_COST AS PROP_COST,B.TOTAL_COST" +
                    //                        " FROM INVT_PRODUCTION_TEMP_MASTER A INNER JOIN INVT_PRODUCTION_TEMP_DETAILS B ON A.PART_NUMBER_SUP = B.MAIN_PART_NUMBER_SUP AND A.TEMP_VERSION = B.TEMP_VERSION AND A.LOC_CODE=B.LOC_CODE WHERE A.PART_NUMBER_SUP = '" + Part_Number_Sup + "' AND A.ZERO_SERIAL = " + ZeroSerial + " AND A.ACTIVE = 'Y' AND B.ACTIVE = 'Y' And A.LOC_CODE='" + Loc_Code + "'");

                    //                    if (rsb != null && rsb.HasRows)
                    //                    {
                    //                        int Itr = 0;
                    //                        decimal PrLCTot = 0;
                    //                        while (rsb.Read())  // CHECK PR & PRR COST ARE OK && EQUAL ? 
                    //                        {
                    //                            PrLCTot = decimal.Parse(rsb["LUP"].ToString());
                    //                            decimal LC = decimal.Parse(rsb["PROP_COST"].ToString()) * (QTY_LUQ - itStock);
                    //                            decimal QTY = decimal.Parse(rsb["PROP_LUQ"].ToString()) * (QTY_LUQ - itStock);

                    //                            int rsbZeroSerial = 0;
                    //                            int rsbPackingOrder = 0;
                    //                            string rsbPriceCode = "";
                    //                            decimal rsbBuyingRate = 0;
                    //                            decimal rsbRetailPrice = 0;
                    //                            GetProductDetails(rsb["Part_Number_Sup"].ToString(), rsb["PACKING"].ToString(), out rsbZeroSerial, out rsbPackingOrder, out rsbPriceCode, out rsbBuyingRate, out rsbRetailPrice);

                    //                            UpdateItemTran(Loc_Code, TranDate, "PRR", rsb["Part_Number_Sup"].ToString(), QTY, LC, 0, int.Parse(rsb["Pack_Multiply_Qty"].ToString()), rsb["PACKING"].ToString(), To_Loc_Code, SysDate, int.Parse(rsb["zero_serial"].ToString()), rsbPackingOrder, rsbRetailPrice, rsbBuyingRate, true, 0, false);
                    //                            UpdateStockBalance(Loc_Code, "PRR", rsb["Part_Number_Sup"].ToString(), QTY, LC, To_Loc_Code, Doc_Date, SysDate, rsbPackingOrder, int.Parse(rsb["zero_serial"].ToString()), rsb["PACKING"].ToString(), rsbBuyingRate, (int)int.Parse(rsb["Pack_Multiply_Qty"].ToString()), true, 0, false);

                    //                            addAdjustmentTransaction(Loc_Code, rsb["Part_Number_Sup"].ToString(), int.Parse(rsb["zero_serial"].ToString()), "PRR", QTY, LC, (int)int.Parse(rsb["Pack_Multiply_Qty"].ToString()), Doc_Date, To_Loc_Code, LC);
                    //                        }
                    //                        addAdjustmentTransaction(Loc_Code, Part_Number_Sup, ZeroSerial, "PRH", QTY_LUQ, PrLCTot * (QTY_LUQ), Pack_Multiply_Qty, Doc_Date, To_Loc_Code, PrLCTot);
                    //                        UpdateTranDtls(Part_Number_Sup, "PRH", Loc_Code, Doc_Date, PrLCTot * (QTY_LUQ - itStock), (QTY_LUQ - itStock), Packing, Doc_No, To_Loc_Code, FOC_LUQ, PackingOrder, ZeroSerial, Pack_Multiply_Qty);
                    //                        UpdateItemTran(Loc_Code, TranDate, "PRH", Part_Number_Sup, (QTY_LUQ - itStock), PrLCTot * (QTY_LUQ - itStock), FOC_LUQ, Pack_Multiply_Qty, Packing, To_Loc_Code, SysDate, ZeroSerial, PackingOrder, Unit_Rate, BUYING_RATE, true, SALE_VAL_WO_TAX, false);
                    //                        UpdateStockBalance(Loc_Code, "PRH", Part_Number_Sup, (QTY_LUQ - itStock), PrLCTot * (QTY_LUQ - itStock), To_Loc_Code, Doc_Date, SysDate, PackingOrder, ZeroSerial, Packing, BUYING_RATE, (int)Pack_Multiply_Qty, true, SALE_VAL_WO_TAX, false);
                    //                    }
                    //                    else Item_Type = "G";
                    //                }
                    //                else if (Doc_Type == "SRT")
                    //                {
                    ...
                    //                            decimal LC = decimal.Parse(rsb["PROP_COST"].ToString()) * (QTY_LUQ);
                    //                            decimal QTY = decimal.Parse(rsb["PROP_LUQ"].ToString()) * (QTY_LUQ);
                    ...
                    //                            UpdateItemTran(Loc_Code, TranDate, "PRH", rsb["Part_Number_Sup"].ToString(), QTY, LC, 0, int.Parse(rsb["Pack_Multiply_Qty"].ToString()), rsb["PACKING"].ToString(), To_Loc_Code, SysDate, int.Parse(rsb["zero_serial"].ToString()), rsbPackingOrder, rsbRetailPrice, rsbBuyingRate, true, 0, false);
                    //                            UpdateStockBalance(Loc_Code, "PRH", rsb["Part_Number_Sup"].ToString(), QTY, LC, To_Loc_Code, Doc_Date, SysDate, rsbPackingOrder, int.Parse(rsb["zero_serial"].ToString()), rsb["PACKING"].ToString(), rsbBuyingRate, (int)int.Parse(rsb["Pack_Multiply_Qty"].ToString()), true, 0, false);
                    //                            addAdjustmentTransaction(Loc_Code, rsb["Part_Number_Sup"].ToString(), int.Parse(rsb["zero_serial"].ToString()), "PRH", QTY, LC, (int)int.Parse(rsb["Pack_Multiply_Qty"].ToString()), Doc_Date, To_Loc_Code, LC);
                    //                        }
                    //                        addAdjustmentTransaction(Loc_Code, Part_Number_Sup, ZeroSerial, "PRR", QTY_LUQ, PrLCTot * (QTY_LUQ), Pack_Multiply_Qty, TranDate, To_Loc_Code, PrLCTot);
                    //                        UpdateItemTran(Loc_Code, TranDate, "PRR", Part_Number_Sup, (QTY_LUQ), PrLCTot * (QTY_LUQ), FOC_LUQ, Pack_Multiply_Qty, Packing, To_Loc_Code, SysDate, ZeroSerial, PackingOrder, Unit_Rate, BUYING_RATE, true, SALE_VAL_WO_TAX, false);
                    //                        UpdateStockBalance(Loc_Code, "PRR", Part_Number_Sup, (QTY_LUQ), PrLCTot * (QTY_LUQ), To_Loc_Code, Doc_Date, SysDate, PackingOrder, ZeroSerial, Packing, BUYING_RATE, (int)Pack_Multiply_Qty, true, SALE_VAL_WO_TAX, false);
                    ...
                    //        {
                    //            decimal itStock = getStock(Part_Number_Sup, Loc_Code, ZeroSerial);
                    //            UpdateItemTran(Loc_Code, TranDate, "PRR", Part_Number_Sup, QTY_LUQ, Val, FOC_LUQ, Pack_Multiply_Qty, Packing, To_Loc_Code, SysDate, ZeroSerial, PackingOrder, Unit_Rate, BUYING_RATE, true, SALE_VAL_WO_TAX, false);
                    //            UpdateStockBalance(Loc_Code, "PRR", Part_Number_Sup, QTY_LUQ, Val, To_Loc_Code, Doc_Date, SysDate, PackingOrder, ZeroSerial, Packing, BUYING_RATE, (int)Pack_Multiply_Qty, true, SALE_VAL_WO_TAX, false);

                    //            if (!clsGen.Exists("select distinct part_number_sup from INVT_PRODUCTION_TEMP_MASTER where part_number_sup='" + Part_Number_Sup + "' and zero_serial=" + ZeroSerial + " and active='Y'  and POST_PRODUCTION='Y'", con, tr))
                    //            {
                    //                if (itStock > 0 && Doc_Type == "PRR" && itStock >= (QTY_LUQ + FOC_LUQ))
                    //                {
                    //                    Item_Type = "G"; // SKIP ENF STOCK IN MAIN
                    //                }
                    //                else if (Doc_Type == "PRR" && itStock < (QTY_LUQ + FOC_LUQ))
                    //                {
                    ...
                    //                        int Itr = 0;
                    //                        decimal PrLCTot = 0;
                    //                        while (rsb.Read())  // CHECK PR & PRR COST ARE OK ? 
                    //                        {
                    //                            PrLCTot = decimal.Parse(rsb["LUP"].ToString());
                    ...
                    //                            UpdateItemTran(Loc_Code, TranDate, "PRR", rsb["Part_Number_Sup"].ToString(), QTY, LC, 0, int.Parse(rsb["Pack_Multiply_Qty"].ToString()), rsb["PACKING"].ToString(), To_Loc_Code, SysDate, int.Parse(rsb["zero_serial"].ToString()), PackingOrder, rsbRetailPrice, rsbBuyingRate, true, 0, false);
                    //                            UpdateStockBalance(Loc_Code, "PRR", rsb["Part_Number_Sup"].ToString(), QTY, LC, To_Loc_Code, Doc_Date, SysDate, PackingOrder, int.Parse(rsb["zero_serial"].ToString()), rsb["PACKING"].ToString(), rsbBuyingRate, (int)int.Parse(rsb["Pack_Multiply_Qty"].ToString()), true, 0, false);
                    //                            addAdjustmentTransaction(Loc_Code, rsb["Part_Number_Sup"].ToString(), int.Parse(rsb["zero_serial"].ToString()), "PRR", QTY, LC, (int)int.Parse(rsb["Pack_Multiply_Qty"].ToString()), Doc_Date, To_Loc_Code, LC);
                    //                        }
                    //                        addAdjustmentTransaction(Loc_Code, Part_Number_Sup, ZeroSerial, "PRR", QTY_LUQ, PrLCTot * (QTY_LUQ), Pack_Multiply_Qty, TranDate, To_Loc_Code, PrLCTot);
                    //                        UpdateItemTran(Loc_Code, TranDate, "PRH", Part_Number_Sup, (QTY_LUQ - itStock), PrLCTot * (QTY_LUQ - itStock), FOC_LUQ, Pack_Multiply_Qty, Packing, To_Loc_Code, SysDate, ZeroSerial, PackingOrder, Unit_Rate, BUYING_RATE, true, SALE_VAL_WO_TAX, false);
                    ...
                    //            if (!clsGen.Exists("select distinct part_number_sup from INVT_PRODUCTION_TEMP_MASTER where part_number_sup='" + Part_Number_Sup + "' and zero_serial=" + ZeroSerial + " and active='Y'  and POST_PRODUCTION='Y'", con, tr))
                    //            {
                    //                if (itStock > 0 && Doc_Type == "GTV" && itStock >= (QTY_LUQ + FOC_LUQ))
                    //                {
                    //                    Item_Type = "G"; // SKIP END STOCK IN MAIN
                    //                }
                    //                else if (Doc_Type == "GTV" && itStock < (QTY_LUQ + FOC_LUQ))
                    //                {
                    //                    OracleDataReader rsb = clsGen.GetValue(con, tr, "SELECT A.TOTAL_UNIT_COST AS LUP,A.LUQ AS MASTERLUQ,A.NET_PRODUCT_COST AS NET_PRODUCT_COST,B.PART_NUMBER_SUP,B.ZERO_SERIAL,B.PACKING,B.PACK_MULTIPLY_QTY,B.LUQ AS SUBLUQ,(B.LUQ/A.QTY) AS PROP_LUQ,B.UNIT_COST AS PROP_COST,B.TOTAL_COST" +
                    //                       " FROM INVT_PRODUCTION_TEMP_MASTER A INNER JOIN INVT_PRODUCTION_TEMP_DETAILS B ON A.PART_NUMBER_SUP = B.MAIN_PART_NUMBER_SUP AND A.TEMP_VERSION = B.TEMP_VERSION AND A.LOC_CODE=B.LOC_CODE WHERE A.PART_NUMBER_SUP = '" + Part_Number_Sup + "' AND A.ZERO_SERIAL = " + ZeroSerial + " AND A.ACTIVE = 'Y' AND B.ACTIVE = 'Y' AND A.LOC_CODE='" + Loc_Code + "'");
                    ...
                    //                            addAdjustmentTransaction(Loc_Code, rsb["Part_Number_Sup"].ToString(), int.Parse(rsb["zero_serial"].ToString()), "PRR", QTY, LC, (int)int.Parse(rsb["Pack_Multiply_Qty"].ToString()), Doc_Date, To_Loc_Code, LC);
                    //                        }
                    //                        addAdjustmentTransaction(Loc_Code, Part_Number_Sup, ZeroSerial, "PRH", QTY_LUQ, PrLCTot * (QTY_LUQ), Pack_Multiply_Qty, TranDate, To_Loc_Code, PrLCTot);
                    //                        UpdateTranDtls(Part_Number_Sup, "PRH", Loc_Code, Doc_Date, PrLCTot * (QTY_LUQ - itStock), (QTY_LUQ - itStock), Packing, Doc_No, To_Loc_Code, FOC_LUQ, PackingOrder, ZeroSerial, Pack_Multiply_Qty);
                    //                        UpdateItemTran(Loc_Code, TranDate, "PRH", Part_Number_Sup, (QTY_LUQ - itStock), PrLCTot * (QTY_LUQ - itStock), FOC_LUQ, Pack_Multiply_Qty, Packing, To_Loc_Code, SysDate, ZeroSerial, PackingOrder, Unit_Rate, BUYING_RATE, true, SALE_VAL_WO_TAX, false);
                    //                        UpdateStockBalance(Loc_Code, "PRH", Part_Number_Sup, (QTY_LUQ - itStock), PrLCTot * (QTY_LUQ - itStock), To_Loc_Code, Doc_Date, SysDate, PackingOrder, ZeroSerial, Packing, BUYING_RATE, (int)Pack_Multiply_Qty, true, SALE_VAL_WO_TAX, false);
                    //                    }
                    ...
                    //        else
                    //        {
                    //            UpdateTranDtls(Part_Number_Sup, Doc_Type, Loc_Code, Doc_Date, Val, QTY_LUQ, Packing, Doc_No, To_Loc_Code, FOC_LUQ, PackingOrder, ZeroSerial, Pack_Multiply_Qty, avg_Lc, false, BUYING_RATE);
                    //            UpdateItemTran(Loc_Code, TranDate, Doc_Type, Part_Number_Sup, QTY_LUQ, Val, FOC_LUQ, Pack_Multiply_Qty, Packing, To_Loc_Code, SysDate, ZeroSerial, PackingOrder, Unit_Rate, BUYING_RATE, true, SALE_VAL_WO_TAX, false);
                    //            UpdateStockBalance(Loc_Code, Doc_Type, Part_Number_Sup, QTY_LUQ, Val, To_Loc_Code, Doc_Date, SysDate, PackingOrder, ZeroSerial, Packing, BUYING_RATE, (int)Pack_Multiply_Qty, true, SALE_VAL_WO_TAX, false);
                    //        }
                    //    }
                    //    catch (Exception ex) { throw; }
                    //    finally { if (rs != null) rs.Close(); if (cmd != null) cmd.Dispose(); }
                    //    break;
                    //#endregion
                    //
                    #endregion
                    case "P":
                        ...
                        try
                        {
                            ...
                            if (Doc_Type == "SAL" || Doc_Type == "SRT")
                            {
                                ...
                                if (!clsGen.Exists("select distinct part_number_sup from INVT_PRODUCTION_TEMP_MASTER where loc_code='" + Loc_Code + "' and part_number_sup='" + Part_Number_Sup + "' and zero_serial=" + ZeroSerial + " and active='Y' and POST_PRODUCTION='Y'", con, tr))
                                {
                                    if (itStock > 0 && Doc_Type == "SAL" && itStock >= QTY_LUQ + FOC_LUQ)
                                    {
                                        Item_Type = "G"; // SKIP END STOCK IN MAIN
                                    ...
                                    else if (Doc_Type == "SAL" && itStock < QTY_LUQ + FOC_LUQ)
                                    {
                                        ...
                                        OracleDataReader rsb = clsGen.GetValue(con, tr, "SELECT A.TOTAL_UNIT_COST AS LUP,A.LUQ AS MASTERLUQ,A.NET_PRODUCT_COST AS NET_PRODUCT_COST,B.PART_NUMBER_SUP,B.ZERO_SERIAL,B.PACKING,B.PACK_MULTIPLY_QTY,B.LUQ AS SUBLUQ,(B.LUQ/A.QTY) AS PROP_LUQ,B.UNIT_COST AS PROP_COST,B.TOTAL_COST" +
                                            " FROM INVT_PRODUCTION_TEMP_MASTER A INNER JOIN INVT_PRODUCTION_TEMP_DETAILS B ON a.loc_code=b.loc_code and A.PART_NUMBER_SUP = B.MAIN_PART_NUMBER_SUP AND A.TEMP_VERSION = B.TEMP_VERSION WHERE a.loc_code='" + Loc_Code + "' and A.PART_NUMBER_SUP = '" + Part_Number_Sup + "' AND A.ZERO_SERIAL = " + ZeroSerial + " AND A.ACTIVE = 'Y' AND B.ACTIVE = 'Y'");
                    ...
                    default:
                        Exception Ex = new Exception("STOCK UPDATION ERROR - INVALID ITEM TYPE " + Part_Number_Sup);
                        throw (Ex);
                }
            }
            catch (Exception Ee)
            {
                throw (new Exception(" UpdateStock " + Loc_Code + "," + Part_Number_Sup + "," + Ee.Message));
            }
            // added on apr 28 10
        ...
        //2022-OCT-12
        public bool ApplayProductionCost(string stProductionItem, OracleConnection oCon, OracleTransaction Tr, decimal RP, string _LocCode)
        {
            decimal ProductionItemRetailPrice = 0;
            decimal ProductionSubItemsNetLandingCost = 0;
            long intRowsAffected = 0;
            string Sql = "";
            try
            {
                if (!clsGen.Exists("select distinct a.part_number_sup,b.zero_serial,a.sub_item_lc,b.lc from INVT_PRODUCTION_TEMP_DETAILS a inner join invt_itemtran_dtls b on a.loc_code=b.loc_code and a.part_number_sup=b.part_number_sup and a.packing=b.packing where a.sub_item_lc <> b.LC  and a.loc_code='" + _LocCode + "' and a.main_Part_number_sup = '" + stProductionItem + "' and a.active='Y'", con, tr))
                    return true;

                ProductionItemRetailPrice = RP;
                Sql = "Select PART_NUMBER_SUP,PACKING,QTY,ZERO_SERIAL,PACK_MULTIPLY_QTY From INVT_PRODUCTION_TEMP_DETAILS Where LOC_CODE='" + _LocCode + "' And main_PART_NUMBER_SUP='" + stProductionItem + "' AND ACTIVE='Y'";
                using (var Cmd = oCon.CreateCommand())
                {
                    Cmd.CommandText = Sql;
                    if (Tr != null) Cmd.Transaction = Tr;
                    using (var Rs = Cmd.ExecuteReader())
                    {
                        if (Rs.HasRows)
                        {
                            ...
                            {
                                intRowsAffected = 0;
                                decimal decLC = decimal.Parse(clsGen.getLandingCost(_LocCode, Rs["PART_NUMBER_SUP"].ToString(), decimal.Parse(Rs["PACK_MULTIPLY_QTY"].ToString()), int.Parse(Rs["ZERO_SERIAL"].ToString()), oCon, Tr));
                                if (decLC <= 0)
                                {
                                    decLC = clsGen.GetBuying(Rs["PART_NUMBER_SUP"].ToString(), Rs["PACKING"].ToString(), oCon, Tr);
                                }

                                ProductionSubItemsNetLandingCost = ProductionSubItemsNetLandingCost + (decLC * decimal.Parse(Rs["QTY"].ToString()));

                                Sql = "Update INVT_PRODUCTION_TEMP_DETAILS Set SUB_ITEM_LC=" + decLC +
                                        " Where LOC_CODE='" + _LocCode + "' And main_PART_NUMBER_SUP ='" + stProductionItem + "' And PART_NUMBER_SUP='" + Rs["PART_NUMBER_SUP"].ToString() + "' And PACKING='" + Rs["PACKING"].ToString() + "' AND ACTIVE='Y'";

                                if (!((clsGen.ExecuteCmd(Sql, out intRowsAffected, oCon, Tr)) && intRowsAffected > 0))
                                {
                                    throw new Exception("Error on Update Production Cost " + stProductionItem + "," + Rs["PART_NUMBER_SUP"].ToString());
                                }
                            }
                        }

                        string OtherExp = "0";
                        clsGen.Exists("select distinct NVL(sum(amount),0) amt from INVT_PRODUCTION_TEMP_EXPDTLS where LOC_CODE='" + _LocCode + "' And MAIN_PART_NUMBER_SUP= '" + stProductionItem + "' and ACTIVE='Y'", 1, out OtherExp, oCon, Tr);

                        if (!Form_Validation.IsPositive(OtherExp))
                        {
                            throw new Exception("Error on Update ProductionCost - Invalid Expense " + stProductionItem + "," + Rs["PART_NUMBER_SUP"].ToString());
                        }

                        string productionTemplateQty = clsGen.GetValue("SELECT QTY FROM INVT_PRODUCTION_TEMP_MASTER WHERE ACTIVE='Y' AND PART_NUMBER_SUP='" + stProductionItem + "'", oCon, Tr);
                        Sql = "Update INVT_PRODUCTION_TEMP_DETAILS Set UNIT_COST=((" + ProductionSubItemsNetLandingCost + "+" + decimal.Parse(OtherExp) + ") / " + decimal.Parse(productionTemplateQty) + " ) * (((QTY* SUB_ITEM_LC * 100) / " + ProductionSubItemsNetLandingCost + ") / 100)  ," +
                           "TOTAL_COST=((((" + ProductionSubItemsNetLandingCost + "+" + decimal.Parse(OtherExp) + ") / " + decimal.Parse(productionTemplateQty) + ") * (((QTY* SUB_ITEM_LC * 100) / " + ProductionSubItemsNetLandingCost + ") / 100))* " + decimal.Parse(productionTemplateQty) + " )" +
                           " Where LOC_CODE='" + _LocCode + "' And main_PART_NUMBER_SUP ='" + stProductionItem + "' And ACTIVE='Y'";
                        ...
                        intRowsAffected = 0;
                        Sql = "Update INVT_PRODUCTION_TEMP_MASTER Set " +
                            "UNIT_COST_SUB=ROUND(" + ProductionSubItemsNetLandingCost + "/QTY,2)," +
                            "TOTAL_COST_SUB=ROUND(" + ProductionSubItemsNetLandingCost + ",2)," +
                            "TOTAL_UNIT_COST=ROUND((" + ProductionSubItemsNetLandingCost + "+" + decimal.Parse(OtherExp) + ")/QTY,2), " +
                            "NET_PRODUCT_COST=ROUND(" + ProductionSubItemsNetLandingCost + "+" + decimal.Parse(OtherExp) + ",2) ," +
                            "TOTAL_EXPENSE=" + decimal.Parse(OtherExp) + "" +
                             " Where LOC_CODE='" + _LocCode + "' And PART_NUMBER_SUP ='" + stProductionItem + "' And ACTIVE='Y'";

                        if (!((clsGen.ExecuteCmd(Sql, out intRowsAffected, oCon, Tr)) && intRowsAffected > 0))
                        {
                            throw new Exception("Error on Update Production cost " + stProductionItem);
                        }
                    }
                }
                return true;
            ...
            {
                throw new Exception(" ApplyProductionCost " + stProductionItem + " - " + Exp.Message);
            }
        }
        public bool GetProductDetails(string Part_Number_Sup, string Packing, out int ZeroSerial, out int PackingOrder, out string price_code, out decimal buying_rate, out decimal RetailPrice)
        {
            using (var cmd = con.CreateCommand())
            {
                if (tr != null) cmd.Transaction = tr;
                cmd.CommandText = "select PRICE_CODE,RETAIL_PRICE,BUYING_RATE,PACKING_ORDER,PART_NUMBER_SUP,ZERO_SERIAL,TAX,packing,costing_method,item_type,pack_multiply_qty,Offer_Pack from INVT_ITEMPACKING where part_number_sup='" + Part_Number_Sup + "' and PACKING='" + Packing + "' AND DEFAULT_PACKING='Y'";
                try
                {
                    using (var rs = cmd.ExecuteReader())
                    {
                        ...
                        {
                            price_code = rs["PRICE_CODE"].ToString();
                            ZeroSerial = int.Parse(rs["ZERO_SERIAL"].ToString());
                            PackingOrder = int.Parse(rs["packing_order"].ToString());
                            buying_rate = decimal.Parse(rs["BUYING_RATE"].ToString());
                            RetailPrice = decimal.Parse(rs["RETAIL_PRICE"].ToString());
                        }
                    }
                }
                catch (Exception eq) { throw (eq); }
        ...
        private bool addAdjustmentTransaction(string Loc_Code, string Part_Number_Sup, int zero_serial, string Doc_Type, decimal QTY_LUQ, decimal Val, decimal Pack_Multiply_Qty, string Doc_Date, string To_Loc_Code, decimal LC)
        {
            try
            {
                string Q1 = "", Q2 = "";
                ...
                {
                    case "SAL":
                        Q1 = "SALE_QTY=SALE_QTY + " + QTY_LUQ + ", SALE_VAL= SALE_VAL + " + Val;
                        i1 = "SALE_QTY,SALE_VAL";
                        break;
                    case "SRT":
                        Q1 = "SALE_R_QTY=SALE_R_QTY + " + QTY_LUQ + ", SALE_R_VAL=SALE_R_VAL + " + Val;
                        i1 = "SALE_R_QTY,SALE_R_VAL";
                        break;
                    case "PRH":
                        Q1 = "PRH_QTY=PRH_QTY+" + QTY_LUQ + ", PRH_VAL=PRH_VAL+" + Val + ",LC=" + LC;
                        i1 = "PRH_QTY,PRH_VAL";
                        break;
                    case "PRR":
                        Q1 = "PRH_R_QTY=PRH_R_QTY+" + QTY_LUQ + ", PRH_R_VAL=PRH_R_VAL+" + Val;
                        i1 = "PRH_R_QTY,PRH_R_VAL";
                        break;
                    case "GTV":
                        Q1 = "TRR_QTY=TRR_QTY+" + QTY_LUQ + ", TRR_VAL=TRR_VAL+" + Val;
                        i1 = "TRR_R_QTY,TRR_VAL";
                        Q2 = "TRI_QTY=TRI_QTY+" + QTY_LUQ + ",TRI_VAL=TRI_VAL+" + Val; // LC chk later
                        i2 = "TRI_QTY,TRI_VAL";
                        break;
                    default:
                        return false; // throw
                ...
                string SQL = "insert into INVT_ITEMTRANSACTION_INTRNLADJ (LOC_CODE,  PART_NUMBER_SUP,  DOC_DATE,zero_serial, PACK_MULTIPLY_QTY, " + i1 + " )";
                string VAl = " Values ('" + Loc_Code + "','" + Part_Number_Sup + "','" + Doc_Date + "'," + zero_serial + "," + Pack_Multiply_Qty + "," + iv + ")";

                string SQL1 = "Update INVT_ITEMTRANSACTION_INTRNLADJ set " + Q1 + " where loc_code='" + Loc_Code + "' and doc_date='" + Doc_Date + "' and part_number_sup='" + Part_Number_Sup + "' and zero_serial=" + zero_serial;
                long rw = 0;
                if (!clsGen.ExecuteCmd(SQL1, out rw, con, tr) || rw <= 0)
                {
                    clsGen.ExecuteCmd(SQL + VAl, con, tr);
                }
                if (Q2 != "")
                {

                    SQL1 = "Update INVT_ITEMTRANSACTION_INTRNLADJ set " + Q2 + " where loc_code='" + To_Loc_Code + "' and doc_date='" + Doc_Date + "' and part_number_sup='" + Part_Number_Sup + "' and zero_serial=" + zero_serial;

                    SQL = "insert into INVT_ITEMTRANSACTION_INTRNLADJ (LOC_CODE,  PART_NUMBER_SUP,  DOC_DATE,zero_serial, PACK_MULTIPLY_QTY ," + i2 + " )";
                    VAl = " Values ('" + To_Loc_Code + "','" + Part_Number_Sup + "','" + Doc_Date + "'," + zero_serial + "," + Pack_Multiply_Qty + "," + iv + ")";
                    if (!clsGen.ExecuteCmd(SQL1, out rw, con, tr) || rw <= 0)
                    {
                        clsGen.ExecuteCmd(SQL + VAl, con, tr);
                    }
        ...
        public bool ApplayBundleValue(string stBundleItem, OracleConnection oCon, OracleTransaction Tr, decimal RP)
        {
            decimal BundleItemRetailPrice = 0;
            decimal BundleSubItemsNetLandingCost = 0;
            OracleCommand Cmd = null;
            ...
            try
            {
                BundleItemRetailPrice = RP;
                if (RP == 0)
                {
                    Sql = "Select RETAIL_PRICE From INVT_ITEMPACKING Where PART_NUMBER_SUP='" + stBundleItem + "' And PACKING_ORDER=1";
                    BundleItemRetailPrice = decimal.Parse(clsGen.GetValue(Sql, oCon, Tr));
                    if (BundleItemRetailPrice <= 0)
                    {
                        Exception ex = new Exception("Error on Update LuggageValue (Price Error)" + stBundleItem);
                        throw ex;
                    }
                }
                Sql = "Select PART_NUMBER_SUP,PACKING,ASY_QTY From INVT_ASSEMBLYSUB Where ASY_PART_NUMBER_SUP='" + stBundleItem + "'";//0291260
                Cmd = oCon.CreateCommand();
                Cmd.CommandText = Sql;
                if (Tr != null) Cmd.Transaction = Tr;
                Rs = Cmd.ExecuteReader();
                if (Rs.HasRows)
                {
                    while (Rs.Read())
                    {
                        Sql = "Select RETAIL_PRICE as SELLINGPRICE,PACK_MULTIPLY_QTY,ZERO_SERIAL From INVT_ITEMPACKING Where PART_NUMBER_SUP='" + Rs["PART_NUMBER_SUP"].ToString() + "' And PACKING='" + Rs["PACKING"].ToString() + "'";
                        Cmd1 = oCon.CreateCommand();
                        Cmd1.CommandText = Sql;
                        ...
                        if (Rs1.HasRows)
                        {
                            ...
                            {
                                intRowsAffected = 0;
                                decimal decLC = decimal.Parse(clsGen.getLandingCost(Defaults.Def_LOC, Rs["PART_NUMBER_SUP"].ToString(), decimal.Parse(Rs1["PACK_MULTIPLY_QTY"].ToString()), int.Parse(Rs1["ZERO_SERIAL"].ToString()), oCon, Tr));//decimal decLC = decimal.Parse(DataConnect.getLandingCost(Defaults.Def_LOC, Rs1["PART_NUMBER_SUP"].ToString(), decimal.Parse(Rs1["PACK_MULTIPLY_QTY"].ToString()), int.Parse(Rs1["ZERO_SERIAL"].ToString()), oCon, Tr));
                                if (decLC <= 0)
                                {
                                    decLC = clsGen.GetBuying(Rs["PART_NUMBER_SUP"].ToString(), Rs["PACKING"].ToString(), oCon, Tr);
                                }
                                BundleSubItemsNetLandingCost = BundleSubItemsNetLandingCost + (decLC * decimal.Parse(Rs["ASY_QTY"].ToString()));
                                Sql = "Update INVT_ASSEMBLYSUB Set SELLINGPRICE=" + Rs1["SELLINGPRICE"].ToString() + ",SPRICETOTAL=(ASY_QTY*(" + Rs1["SELLINGPRICE"].ToString() + ")),ITEM_BUNDLE_COST=(ASY_QTY*(" + decLC + "))" +
                                      " Where ASY_PART_NUMBER_SUP ='" + stBundleItem + "' And PART_NUMBER_SUP='" + Rs["PART_NUMBER_SUP"].ToString() + "' And PACKING='" + Rs["PACKING"].ToString() + "'";
                                if (!((clsGen.ExecuteCmd(Sql, out intRowsAffected, oCon, Tr)) && intRowsAffected > 0))
                                {
                                    Exception ex = new Exception("Error on Update Luggagecost " + stBundleItem + "," + Rs["PART_NUMBER_SUP"].ToString());
                                    throw ex;
                                }
                            }
                        }
                        ...
                        {
                            Exception ex = new Exception("Error on Update BundleValue (Sub item not Found-itempacking) " + stBundleItem);
                            throw ex;
                        }
                    }
                    ...
                    //decimal ItemContributionPercent = ((SubItemAssignQty * SubItemLC) * 100) / NewItemLC;
                    //decimal BunValue = (NewBundleItemRetPrice * ItemContributionPercent) / 100;
                    //ItemBundleValue = Math.Round(BunValue, 2);
                    //return true;
                    //__________________________________________________________

                    intRowsAffected = 0;
                    Sql = "Update INVT_ASSEMBLYSUB Set ITEM_BUNDLE_VALUE=" +
                          "ROUND(((" + BundleItemRetailPrice + "*(((ITEM_BUNDLE_COST)*100)/" + BundleSubItemsNetLandingCost + "))/100),2)" +
                          " Where ASY_PART_NUMBER_SUP ='" + stBundleItem + "'";
                    if (!((clsGen.ExecuteCmd(Sql, out intRowsAffected, oCon, Tr)) && intRowsAffected > 0))
                    {
                        Exception ex = new Exception("Error on Update Luggagecost " + stBundleItem);
                        throw ex;
                    }
                    Rs.Close();
                    return true;
                ...
                {
                    Exception ex = new Exception("Error on Update LuggageValue Sub item not Found-Assemblysub " + stBundleItem);
                    throw ex;
                }
            }
            catch (Exception Exp)
            {
                Exception ex = new Exception(" ApplyBundleValue " + stBundleItem + " - " + Exp.Message);
                throw ex;
            }
            finally
            {
        ...
        public bool AddToTranStock(string Loc_Code, string Part_Number_Sup, string Packing, string Item_Type, string Doc_No, string Doc_Type, string Grv_Type, decimal QTY_LUQ, decimal Val, decimal FOC_LUQ, decimal PackQty, decimal Pack_Multiply_Qty, string Doc_Year, string Doc_Date, string To_Loc_Code, OracleConnection con, OracleTransaction tr, string TranBaseLoc, decimal Unit_Price, bool UPDATE_LC_ONLY, string OthrFromDt = null, string OthrToDate = null)
        {

            string sql1 = "", val1 = "";
            if (OthrFromDt != null && DataConnector.IsDate(OthrFromDt))
            {
                sql1 = ",OTHR_FROM_DATE";
                val1 = ",'" + DateTime.Parse(OthrFromDt).ToString("dd/MMM/yyyy") + "'";
            }
            if (OthrToDate != null && DataConnector.IsDate(OthrToDate))
            {
                sql1 += ",OTHR_TO_DATE";
                val1 += ",'" + DateTime.Parse(OthrToDate).ToString("dd/MMM/yyyy") + "'";
            }
            ...
            if (Defaults.Def_MAIN_LOCATION)
            {
                ...
                using (var cmd = con.CreateCommand())
                {
                    ...
                    using (var rs = cmd.ExecuteReader())
                    {
                        while (rs.Read())
                        {
                            if (!clsGen.insertTable("TRAN_STOCK", "LOC_CODE,PART_NUMBER_SUP,PACKING,ITEM_TYPE,DOC_NO,DOC_TYPE,GRV_TYPE,LUQ,PRODUCT_VALUE,FOC_LUQ,PACKQTY,PACK_MULTIPLY_QTY,DOC_YEAR,DOC_DATE,TO_LOC,TRAN_BASE_LOC,TRAN_TO_LOC,UNIT_PRICE,SKIP_LC,ONLY_LC_UPDATE,price_code" + sql1, "'" + Loc_Code + "','" + Part_Number_Sup + "','" + Packing + "','" + Item_Type + "','" + Doc_No + "','" + Doc_Type + "','" + Grv_Type + "'," + QTY_LUQ + "," + Val + "," + FOC_LUQ + "," + PackQty + "," + Pack_Multiply_Qty + ",'" + Doc_Year + "','" + DateTime.Parse(Doc_Date).ToString("dd/MMM/yyyy") + "','" + To_Loc_Code + "','" + TranBaseLoc + "','" + rs["loc_code"].ToString() + "'," + Unit_Price + ",'" + (Skip_LC_Update ? "Y" : "N") + "','" + (UPDATE_LC_ONLY ? "Y" : "N") + "','" + Price_Code + "'" + val1, con, tr))
                            {
                                throw (new Exception("Cannot Insert To " + rs["loc_code"].ToString()));
                            }
                        }
            ...
            {
                clsGen.insertTable("TRAN_STOCK", "LOC_CODE,PART_NUMBER_SUP,PACKING,ITEM_TYPE,DOC_NO,DOC_TYPE,GRV_TYPE,LUQ,PRODUCT_VALUE,FOC_LUQ,PACKQTY,PACK_MULTIPLY_QTY,DOC_YEAR,DOC_DATE,TO_LOC,TRAN_BASE_LOC,TRAN_TO_LOC,UNIT_PRICE,SKIP_LC,ONLY_LC_UPDATE,price_code" + sql1, "'" + Loc_Code + "','" + Part_Number_Sup + "','" + Packing + "','" + Item_Type + "','" + Doc_No + "','" + Doc_Type + "','" + Grv_Type + "'," + QTY_LUQ + "," + Val + "," + FOC_LUQ + "," + PackQty + "," + Pack_Multiply_Qty + ",'" + Doc_Year + "','" + DateTime.Parse(Doc_Date).ToString("dd/MMM/yyyy") + "','" + To_Loc_Code + "','" + TranBaseLoc + "','" + Defaults.Def_Main_Loc + "'," + Unit_Price + ",'" + (Skip_LC_Update ? "Y" : "N") + "','" + (UPDATE_LC_ONLY ? "Y" : "N") + "','" + Price_Code + "'" + val1, con, tr);
            }
            return true;
        }
        ...
        public bool ApplayLuggageValue(string stLuggageItem, out decimal LuggageMainItemRetailPrice)//public bool ApplayLuggageValue(string stLuggageItem, decimal LuggageSubItemQty, decimal LuggageSubItemRetailPrice, decimal LuggageSubItemsNetRetailPrice, decimal LuggageMainItemRetailPrice, OracleConnection oCon, OracleTransaction Tr)
        {
            ...
            try
            {
                ...
                {
                    Exception ex = new Exception("Error on Update LuggageValue (Price Error) " + stLuggageItem);
                    throw (ex);
                }
                ...
                if (Rs.HasRows)
                {
                    while (Rs.Read())
                    {
                        ...
                        if (Rs1.HasRows)
                        {
                            if (Rs1.Read())
                            {
                                ...
                                {
                                    Exception ex1 = new Exception("Error on Update Luggagecost " + Rs["PART_NUMBER_SUP"].ToString());
                                    throw (ex1);
                                }
                            }
                        }
                        ...
                        {
                            Exception ex = new Exception("Missing Sub Item LuggageValue - itempacking " + stLuggageItem + " " + Rs["PART_NUMBER_SUP"].ToString());
                            throw (ex);
                        }
                    }
                    //Sql = "Update INVT_ASSEMBLYSUB Set ITEM_BUNDLE_VALUE=" + "(" + newLandingCost + "*(ITEM_BUNDLE_VALUE*100/" + RetailPrice + ")/100)" + " Where ASY_PART_NUMBER_SUP ='" + stLuggageItem + "'";
                    ...
                    if (!((clsGen.ExecuteCmd(Sql, out intRowsAffected, con, tr)) && intRowsAffected > 0))
                    {
                        Exception ex = new Exception("Error on Update Luggagecost " + stLuggageItem);
                        throw (ex);
                    }
                    Rs.Close();
                    return true;
                ...
                {
                    Exception ex = new Exception("Error on Update LuggageValue ( sub item not found assemblysub) " + stLuggageItem);
                    throw (ex);
                }
            }
            catch (Exception Exp)
            {
                Exp = new Exception(Exp.Message + " ApplyLugguageValue " + stLuggageItem);
                throw (Exp);
            }
            finally
            {
        ...
        public bool ApplayLuggageCost(string stLuggageItem, decimal RetailPrice, decimal newLandingCost)
        {
            ...
            {
                Exp = new Exception("Error on Update Luggagecost " + stLuggageItem + " " + Exp.Message);
                throw (Exp);
            }
            return false;
        }
        ...
        private bool UpdateItemTran(string Loc_Code, string Doc_Date, string Doc_Type, string Part_Number_Sup, decimal Qty, decimal Val, decimal FOC, decimal Pack_Multiply_Qty, string Packing, string To_Loc, string SysDate, int ZeroSerial, int PackingOrder, decimal Unit_Rate, decimal Buying_Rate, bool ThrowException = true, decimal SALE_VAL_WO_TAX = 0, bool SKIP_LC_UPD_OF_ITEM = false)
        {
            ...
            try
            {
                ...
                switch (Doc_Type.ToUpper())
                {
                    ...
                    case "PRH":
                        ...
                        //+ ",BUYING_RATE=round((case when BUYING_RATE<=0 or cb_qty<=0 or cb_qty/pack_multiply_qty+" + Qty + "/pack_multiply_qty=0  then " + ((Val * Pack_Multiply_Qty) / Qty) + " else (BUYING_RATE*CB_QTY/pack_multiply_qty + " + (((Val * Pack_Multiply_Qty) / Qty) * Qty / Pack_Multiply_Qty) + ")/( cb_qty/pack_multiply_qty +" + Qty / Pack_Multiply_Qty + ")  end),2 )";
                        + ",BUYING_RATE=0";
                    ...
                    case "GTV":
                    case "ADJ":
                        Sql1 = " TRI_QTY=TRI_QTY+" + Qty + ",TRI_val=TRI_val+" + Val + ",Offer_Pack='" + offer_pack + "',PRICE_CODE='" + Price_Code + "',CB_QTY=CB_QTY+" + Qty + (ModifyLc ? ",LC=" + LC_ForTranDtl : "") + " where loc_code='" + To_Loc + "' and doc_date='" + Doc_Date + "' and part_number_Sup='" + Part_Number_Sup + "' and zero_serial='" + ZeroSerial + "'";
                        Sql = Sql + " set TRR_QTY=TRR_QTY+" + Qty + ",TRR_val=TRR_val+" + Val + ",Offer_Pack='" + offer_pack + "',PRICE_CODE='" + Price_Code + "'";
                        AQty = -1 * Qty;
                        break;
                    default:
            ...
            ReSl: sql = Sql + " " + upd + " where doc_date='" + Doc_Date + "' and  loc_code='" + Loc_Code + "'  and part_number_Sup='" + Part_Number_Sup + "' and zero_serial='" + ZeroSerial + "'";
                ...
                if (Result == 0)
                {
                    ...
                    {
                        if (Lc_Loc <= 0) Lc_Loc = Buying_Rate / Pack_Multiply_Qty;
        ...
        private decimal getStock(string Part_Number_Sup, string Loc_Code, int ZeroSerial)
        {
            ...
            OracleCommand cmd = null; OracleDataReader rs = null;
            try
            {
                cmd = con.CreateCommand();
                cmd.Transaction = tr;
        ...
        private bool UpdateTranDtls(string Part_Number_Sup, string Doc_Type, string LOC_CODE, string Doc_Date, decimal Val, decimal Qty, string Packing, string Doc_NO, string To_LOC, decimal Foc, int PackingOrder, int ZeroSerial, decimal Pack_Multi, decimal avg_Lc = 0, bool SKIP_LC_UPD_OF_ITEM = false, decimal BUYING_RATE = 0)
        {
            ...
            try
            {
                if (Doc_Type == "PRH" || Doc_Type == "GTV")
                {
                    ...
                    if (Defaults.G_LC_CALCULATION.Equals("WEIGHTEDAVG"))
                    {
                        ...
                        if (OLDLC == 0 && Defaults.G_LCCALCULATION_OLDLCZERO == "BUYING_RATE")
                            OLDLC = Buying_rate;
                        else if (OLDLC == 0 && Defaults.G_LCCALCULATION_OLDLCZERO == "NEW_LC")
                            skipCalculation = true;
                        if (Stck < 0)
                            Stck = 0;
                        if ((Stck + Qty) != 0 && OLDLC != 0 && !skipCalculation)
                        {
                ...
                switch (Doc_Type.ToUpper())
                {
                    ...
                    case "GTV":
                        ...
                        if (LCDebDateTo != "" && LCDateTo != "" && DateTime.Parse(LCDateTo) < DateTime.Parse(LCDebDateTo))
                        {
            ...
            {
                ex = (new Exception("UpdateTranDtls " + Part_Number_Sup + " " + LOC_CODE + " " + ex.Message));
                throw ex;
            }
            finally
            {
            ...
            if (Result <= 0)
            {
                Exception ex = new Exception("Cannot Update Stock. UpdateTranDtls " + Part_Number_Sup + " " + LOC_CODE);
                throw (ex);
            }
            else
            {
        ...
        */
        public bool UpdateLocLcs(string LOC_CODE, decimal LC, string Part_Number_Sup, int ZeroSerial, int PackingOrder, string Packing, string DATE, string DOC_TYPE, string DOC_NO, string BaseLoc, decimal avg_LC = 0, string LC_DATE_FROM = null, string LC_DATE_TO = null, bool SKIP_LC_UPD_OF_ITEM = false)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    while (rs.Read())
                    {
                        ...
                        if (LC_DATE_FROM == null && LC_DATE_TO == null)
                        {
                            string DT = "";
                            if (DOC_TYPE == "PRH")
                            {
                            ...
                            {
                                DT = ",LAST_DBN_NO='" + DOC_NO + "',LAST_DBN_DATE='" + DateTime.Parse(DATE.ToString()).ToString("dd/MMM/yyyy") + "',LAST_DBN_LC=" + LC + "";
                            }
                            ...
                            if (clsGen.updateTable("invt_itemtransaction", "LC=" + LC, "loc_code='" + rs["loc_code"].ToString() + "' and doc_date>='" + DateTime.Parse(DATE.ToString()).ToString("dd/MMM/yyyy") + "' and part_number_Sup='" + Part_Number_Sup + "' and zero_serial=" + ZeroSerial, out roweffect, con, tr) && roweffect <= 0)
                            { }
                        }
                        else
                        {
                            if (clsGen.updateTable("invt_itemtransaction", "LC=" + LC, "loc_code='" + rs["loc_code"].ToString() + "' and doc_date>='" + DateTime.Parse(LC_DATE_FROM.ToString()).ToString("dd/MMM/yyyy") + "' and doc_date<='" + DateTime.Parse(LC_DATE_TO.ToString()).ToString("dd/MMM/yyyy") + "' and part_number_Sup='" + Part_Number_Sup + "' and zero_serial=" + ZeroSerial, out roweffect, con, tr) && roweffect <= 0)
                            { }
                        }
                    }
                    return true;
                ...
                {
                    ex = new Exception("Cannot Update Stock. UpdateLocLcs " + Part_Number_Sup + " " + LOC_CODE + "  " + ex.Message);
                    return false;
                }
                finally
                {
        ...
        private decimal getLC(string Loc_Code, string Part_Number_sup, int Zero_Serial, string Packing)
        {
            try
            {
                ...
                if (!clsGen.IsNumeric1(Lc) || Lc == "" || Lc == "0")
                {
        ...
        private bool UpdateStockBalance(string Loc_code, string Doc_type, string part_number_Sup, decimal Qty, decimal Val, string To_Loc_Code, int ZeroSerial, int PackingOrder, string Packing, decimal BUYING_RATE, int Pack_Multiply_Qty, decimal SALE_VAL_WO_TAX = 0, bool SKIP_LC_UPD_OF_ITEM = false)
        {
            int Result = -1;
            OracleCommand cmd = null;
            try
            {
                ...
                {
                    case "SAL":
                        Sql = Sql + " set YR_SALE_QTY=YR_SALE_QTY + " + Qty + ",YR_SALE_VAL=YR_SALE_VAL + " + Val + ",SALE_VAL_WO_TAX=SALE_VAL_WO_TAX+" + SALE_VAL_WO_TAX + " ,SALE_QTY=sale_qty+" + Qty + ",sale_val=sale_val+" + Val + ",cb_qty=cb_qty-(" + Qty + "),cb_val=cb_val-(" + Val + ")";
                        break;
                    case "SRT":
                        Sql = Sql + " set YR_SALE_R_QTY=YR_SALE_R_QTY + " + Qty + ",YR_SALE_R_VAL=YR_SALE_R_VAL + " + Val + ",SALE_VAL_WO_TAX=SALE_VAL_WO_TAX+" + (-1 * SALE_VAL_WO_TAX) + ", SALE_R_QTY=sale_R_qty+" + Qty + ",sale_R_val=sale_R_val+" + Val + ",cb_qty=cb_qty+" + Qty + ",cb_val=cb_val+" + Val;
                        break;
                    case "PRH":
                        ...
                        + ",AVG_LC=round((case when lc<=0 or cb_qty<=0 or cb_qty+" + Qty + "<=0 then " + LC_ForTranDtl + " else (LC*CB_QTY + " + (LC_ForTranDtl * Qty) + ")/( cb_qty+" + Qty + ")  end),2  )"
                        //+ ",BUYING_RATE=(case when BUYING_RATE<=0 or cb_qty<=0 or cb_qty/pack_multiply_qty+" + Qty + "/pack_multiply_qty=0  then " + ((Val * Pack_Multiply_Qty) / Qty) + "  else (BUYING_RATE*CB_QTY/pack_multiply_qty + " + (((Val * Pack_Multiply_Qty) / Qty) * Qty / Pack_Multiply_Qty) + ")/( cb_qty/pack_multiply_qty +" + Qty / Pack_Multiply_Qty + ")  end  )";
                        + ",BUYING_RATE=0";
                        break;
                    case "PRR":
        ...
        private bool UpdateStockBalance(string Loc_code, string Doc_type, string part_number_Sup, decimal Qty, decimal Val, string To_Loc_Code, string Doc_Date, string SysDate, int packingorder, int ZeroSerial, string Packing, decimal BUYING_RATE, int Pack_Multiply_Qty, bool ThrowException = true, decimal SALE_VAL_WO_TAX = 0, bool SKIP_LC_UPD_OF_ITEM = false)
        {
            ...
            return UpdateStockBalance(Loc_code, Doc_type, part_number_Sup, Qty, Val, To_Loc_Code, ZeroSerial, packingorder, Packing, BUYING_RATE, Pack_Multiply_Qty, SALE_VAL_WO_TAX);
            ...
            try
            {
                ...
                switch (Doc_type.ToUpper())
                {
                    ...
                    case "ADJ":
                        ...
                        break;
                    default:
                        Exception ex = new Exception("Invalid Document Type. Stock Balance" + Loc_code + part_number_Sup + Doc_type);
                        throw (ex);
                }
            ...
            ReSl:
                ...
                if (Result == 0)
                {
                    ...
                    cmd.Dispose();
                    if (Result > 0) goto ReSl;
                    Exception ex = new Exception("Cannot Update New Item Balance Stock. UpdateBalanceStock" + part_number_Sup + " " + Packing + " " + Loc_code);
                    throw (ex);
                }
                if (Result > 0 && !(Sql1.Equals("")))
                {
                    ...
                    while (Yr++ <= DateTime.Parse(SysDate).Year)
                    {
                        ...
                        re21:
                            ...
                            if (Result == 0)
                            {
                                ...
                                {
                                    Exception ex = new Exception("Cannot Update New Item Balance Stock(Month). UpdateBalanceStock" + part_number_Sup + " " + Packing + " " + Loc_code);
                                    throw (ex);
                                    // Error Case
                                }
                            }
                        re22:
                            ...
                            if (Result == 0)
                                ...
                                {
                                    Exception ex = new Exception(" UpdateBalanceStock Cannot Update New Item Balance Stock(" + Month + ")." + part_number_Sup + " " + Packing + " " + Loc_code);
                                    throw (ex);
                                }
                        }
                        if (Yr < DateTime.Parse(SysDate).Year) cMonth = 12;
        ...
        private string Get_ItemType(string PART_NUMBER_SUP, out string Link_Code, string Costing_Method)
        {
            ...
            {
                throw (new Exception("GetItemType " + PART_NUMBER_SUP + " Link " + Link_Code + " " + eq.Message));
            }
            finally
            {
                cmd.Dispose();
            ...
            try
            {
                ...
                switch (Doc_type.ToUpper())
                {
                    ...
                    case "ADJ":
                        ...
                        Sql1 = Sql + " set YR_TRI_QTY=YR_TRI_QTY+" + Qty + ",YR_TRI_val=YR_TRI_val+" + Val + ",TRI_QTY=TRI_QTY+" + Qty + ",TRI_val=TRI_val+" + Val + ",cb_qty=cb_qty+" + Qty + ",cb_val=cb_val+" + Val + updlc + " where loc_code='" + To_Loc_Code + "' and part_number_Sup='" + part_number_Sup + "' and BATCH_CODE = '" + batchCode + "' and zero_serial=" + ZeroSerial;
                        Sql = Sql + " set YR_TRR_QTY=YR_TRR_QTY+" + Qty + ",YR_TRR_val=YR_TRR_val+" + Val + ",TRR_QTY=TRR_QTY+" + Qty + ",TRR_val=TRR_val+" + Val + ",cb_qty=cb_qty-(" + (Qty) + "),cb_val=cb_val-(" + (Val) + ")";
        ...
        public decimal getStockBatch(string LocCode, string PartNumberSup, int ZeroSerial, string batchCode, OracleConnection Conn, OracleTransaction Tr)
        {
            ...
            try
            {
                ...
                cmd.CommandText = "Select Round((CB_QTY),2) as STOCK From INVT_INVENTORYBALANCE_BATCH Where LOC_CODE='" + LocCode + "' And PART_NUMBER_SUP='" + PartNumberSup + "' And ZERO_SERIAL=" + ZeroSerial + " AND BATCH_CODE = '" + batchCode + "' AND CB_QTY >0";
                if (string.IsNullOrEmpty(batchCode))
                {
                    ...
                    cmd.CommandText = "Select Round (SUM(CB_QTY),2) as STOCK From INVT_INVENTORYBALANCE_BATCH Where LOC_CODE='" + LocCode + "' And PART_NUMBER_SUP='" + PartNumberSup + "' And ZERO_SERIAL=" + ZeroSerial + " AND CB_QTY >0";
                }
                rs = cmd.ExecuteReader();
                if (rs.HasRows)
                {
        ...
        public List<ModelBatch> GetBatchStockList(string partNumberSup, int zeroSerial, string locCode, OracleConnection Con, OracleTransaction Tr)
        {
            ...
            try
            {
                ...
                oSql = "Select LOC_CODE,ROUND((OB_QTY/PACK_MULTIPLY_QTY),4) as OB_QTY,((PRH_QTY-PRH_R_QTY)/PACK_MULTIPLY_QTY) as PRHQTY," +
                       ...
                       "ROUND((CB_QTY/PACK_MULTIPLY_QTY),4)as CB_QTY,(ADJ_QTY/PACK_MULTIPLY_QTY)as ADJ_QTY,ZERO_SERIAL,PACKING,PACK_MULTIPLY_QTY,BATCH_CODE,BATCH_EXPIRY " +
                ...
                else
                {
                    ...
                    cmd.CommandText = "SELECT BATCH_CODE,CB_QTY,BATCH_EXPIRY EXPIRY_DATE FROM INVT_INVENTORYBALANCE_BATCH WHERE PART_NUMBER_SUP='" + partNumberSup + "' AND CB_QTY > 0 AND ZERO_SERIAL=" + zeroSerial + "  AND LOC_CODE='" + locCode + "' ORDER BY  EXPIRY_DATE ASC ";
                    reader = cmd.ExecuteReader();
        ...
        private bool UpdateItemTranBatch(string Loc_Code, string Doc_Date, string Doc_Type, string Part_Number_Sup, decimal Qty, decimal Val, decimal FOC, decimal Pack_Multiply_Qty, string Packing, string To_Loc, string SysDate, int ZeroSerial, int PackingOrder, decimal Unit_Rate, string batchCode)
        {
            ...
            try
            {
                ...
                switch (Doc_Type.ToUpper())
                {
                    ...
                    case "GTV":
                    case "ADJ":
                        Sql1 = " TRI_QTY=TRI_QTY+" + Qty + ",offer_pack='" + offer_pack + "',price_code='" + Price_Code + "',TRI_val=TRI_val+" + Val + ",CB_QTY=CB_QTY+" + Qty + (ModifyLc ? ",LC=" + LC_ForTranDtl : "") + " where loc_code='" + To_Loc + "' and doc_date='" + Doc_Date + "' and part_number_Sup='" + Part_Number_Sup + "' and zero_serial='" + ZeroSerial + "' and BATCH_CODE ='"+batchCode+"'";
            ...
            ReSl: sql = Sql + " " + upd + " where doc_date='" + Doc_Date + "' and  loc_code='" + Loc_Code + "'  and part_number_Sup='" + Part_Number_Sup + "' and zero_serial='" + ZeroSerial + "' AND BATCH_CODE = '" + batchCode + "'";
                ...
                if (Result == 0)
                {
                    ...
                    if (ZeroSerial == 0 && PackingOrder != 0)
                    {
                        cmd.CommandText = "insert into INVT_ITEMTRANSACTION_BATCH(doc_date,loc_code,doc_year,doc_month,doc_day,part_number_sup,part_number,PART_DESCRIPTION,CATEGORY_CODE,BRAND_CODE,CATEGORY_NAME,BRAND_NAME,SUPPLIER_CODE,SUPPLIER_NAME,LINKED_ITEM,packing,pack_multiply_qty,ob_qty,zero_serial,CB_QTY,LC,division_code,division_name,BATCH_CODE) select  '" + Doc_Date + "' as doc_date,'" + Loc_Code + "' as loc_code,'" + Dt.Year + "' as doc_year," + Dt.Month + " as Doc_month," + Dt.Day + " as doc_day,'" + Part_Number_Sup + "' as part_number_sup,part_number,PART_DESCRIPTION as part_description,CATEGORY_CODE,BRAND_CODE,CATEGORY_NAME,BRAND_NAME,SUPPLIER_CODE,SUPPLIER_NAME,LINKED_ITEM,default_packing,1," + Ob_Qty + " as ob_qty,0 as zero_serial," + Ob_Qty + " as Cb_qty," + Lc_Loc + " as LC,division_code,division_name,'"+ batchCode + "' as BATCH_CODE from invt_inventorymaster where part_number_sup='" + Part_Number_Sup + "'";
                    }
                    else
                    {
                        cmd.CommandText = "insert into INVT_ITEMTRANSACTION_BATCH(doc_date,loc_code,doc_year,doc_month,doc_day,part_number_sup,part_number,PART_DESCRIPTION,CATEGORY_CODE,BRAND_CODE,CATEGORY_NAME,BRAND_NAME,SUPPLIER_CODE,SUPPLIER_NAME,LINKED_ITEM,packing,pack_multiply_qty,ob_qty,zero_serial,CB_QTY,LC,division_code,division_name,BATCH_CODE) select  '" + Doc_Date + "' as doc_date,'" + Loc_Code + "' as loc_code,'" + Dt.Year + "' as doc_year," + Dt.Month + " as Doc_month," + Dt.Day + " as doc_day,'" + Part_Number_Sup + "' as part_number_sup,part_number,PART_DESCRIPTION as part_description,CATEGORY_CODE,BRAND_CODE,CATEGORY_NAME,BRAND_NAME,SUPPLIER_CODE,SUPPLIER_NAME,LINKED_ITEM,'" + Packing + "'," + Pack_Multiply_Qty + "," + Ob_Qty + " as ob_qty," + ZeroSerial + " as zero_serial," + Ob_Qty + " as Cb_qty," + Lc_Loc + " as LC,division_code,division_name,'"+batchCode+"' as BATCH_CODE  from invt_inventorymaster where part_number_sup='" + Part_Number_Sup + "'";
                    }
                    Result = cmd.ExecuteNonQuery();
                    cmd.Dispose();
                    if (Result > 0) goto ReSl;
