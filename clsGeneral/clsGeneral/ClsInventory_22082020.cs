...
namespace MALL
{
    public class ClsInventory
    {
        ...
        */
        ...
        public bool UpdateStock(string Loc_Code, string Part_Number_Sup, string Packing, string Item_Type, string Doc_No, string Doc_Type, string GRV_TYPE, decimal QTY_LUQ, decimal Val, decimal FOC_LUQ, decimal PackQty, decimal Pack_Multiply_Qty, string Doc_Year, string Doc_Date, string To_Loc_Code, decimal Unit_Rate, bool boolAddToTranStock, string Price_Code, decimal BUYING_RATE)
        {
            ...
            try
            {
                ...
                if (BUYING_RATE == 0) BUYING_RATE = Buying_rate;
                    ...
                    case "B":
                        ...
                        try
                        {
                            ...
                            while (rs.Read())
                            {
                                ...
                                SubItemCount++;
                                UpdateStock(Loc_Code, rs["part_number_sup"].ToString(), rs["packing"].ToString(), "", Doc_No, Doc_Type, "", (QTY_LUQ * rs.GetDecimal(0) * rs.GetDecimal(1)), (QTY_LUQ * rs.GetDecimal(2)) - Corr, (FOC_LUQ * rs.GetDecimal(0) * rs.GetDecimal(1)), rs.GetDecimal(1), rs.GetDecimal(0), Doc_Year, Doc_Date, To_Loc_Code, Unit_Rate, false, Price_Code, BUYING_RATE);//UpdateStock(Loc_Code, rs["part_number_sup"].ToString(), rs["packing"].ToString(), "", Doc_No, Doc_Type, "", (QTY_LUQ * rs.GetDecimal(0) * rs.GetDecimal(1)),(QTY_LUQ * rs.GetDecimal(1) * rs.GetDecimal(2) * rs.GetDecimal(0)), FOC_LUQ, rs.GetDecimal(1), rs.GetDecimal(0), Doc_Year, Doc_Date, To_Loc_Code);
                            }
                            if (SubItemCount <= 0)
                            {
        ...
        private bool UpdateItemTran(string Loc_Code, string Doc_Date, string Doc_Type, string Part_Number_Sup, decimal Qty, decimal Val, decimal FOC, decimal Pack_Multiply_Qty, string Packing, string To_Loc, string SysDate, int ZeroSerial, int PackingOrder, decimal Unit_Rate, decimal Buying_Rate)
        {
            //
            // 10 , 10*5
            ...
            try
            {
                ...
                switch (Doc_Type.ToUpper())
                {
                    case "SAL":
                        ...
                        Sql = Sql + " set SALE_QTY=sale_qty+" + Qty + ",sale_val=sale_val+" + Val + ",PRICE_CODE='" + Price_Code + "',Offer_Pack='" + offer_pack + "'";
                    ...
                    case "SRT":
                        ...
                        Sql = Sql + " set SALE_R_QTY=sale_R_qty+" + Qty + ",sale_R_val=sale_R_val+" + Val + ",PRICE_CODE='" + Price_Code + "',Offer_Pack='" + offer_pack + "'";
                    ...
                    case "PRH":
                        ...
                        + ",AVG_LC=(case when lc<=0 or cb_qty<=0 or cb_qty+" + Qty + "=0 then " + LC_ForTranDtl + " else (LC*CB_QTY + " + (LC_ForTranDtl * Qty) + ")/( cb_qty+" + Qty + ")  end  )"
        ...
        */
        ...
        private bool UpdateStockBalance(string Loc_code, string Doc_type, string part_number_Sup, decimal Qty, decimal Val, string To_Loc_Code, int ZeroSerial, int PackingOrder, string Packing, decimal BUYING_RATE, int Pack_Multiply_Qty)
        {
            int Result = -1;
            OracleCommand cmd = null;
            try
        ...
        private bool UpdateStockBalance(string Loc_code, string Doc_type, string part_number_Sup, decimal Qty, decimal Val, string To_Loc_Code, string Doc_Date, string SysDate, int packingorder, int ZeroSerial, string Packing, decimal BUYING_RATE, int Pack_Multiply_Qty)
        {
            ...
            return UpdateStockBalance(Loc_code, Doc_type, part_number_Sup, Qty, Val, To_Loc_Code, ZeroSerial, packingorder, Packing, BUYING_RATE, Pack_Multiply_Qty);
